import type {
  WorkReportSignalPacketExportFormat,
  WorkReportSignalPacketPortable,
...
export function exportWorkReportSignalPacket(
  packet: WorkReportSignalPacketPortable,
  format: WorkReportSignalPacketExportFormat
): WorkReportSignalPacketExportArtifact {
...
export function buildWorkReportSignalPacketMarkdown(packet: WorkReportSignalPacketPortable) {
  ...
  packet.runs.forEach((entry) => {
    ...
    lines.push(
      `- Summary: ${entry.run.result?.summary ?? "AI run has not returned a summary yet."}`
    );
...
function buildFileBaseName(packet: WorkReportSignalPacketPortable) {
