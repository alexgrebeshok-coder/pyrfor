...
// Mock Prisma singleton ДО импорта команд
vi.mock('@/lib/prisma', () => ({
  prisma: {
    project: {
      findMany: vi.fn(() => [
...
describe('Telegram Bot Commands', () => {
  ...
  describe('/start command', () => {
    it('should send welcome message', async () => {
      ...
      expect(mockBot.sendMessage).toHaveBeenCalledWith(
        chatId,
        expect.stringContaining('CEOClaw')
      );
    });
  });
  ...
  describe('/add_task command', () => {
    it('should create new task', async () => {
      ...
      const mockBot = { sendMessage: vi.fn() };
      const chatId = 123456;
      const match = ['/add_task ЧЭМК Тестовая задача', 'ЧЭМК Тестовая задача'] as unknown as RegExpExecArray;
