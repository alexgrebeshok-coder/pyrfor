...
import type { AIApplyExecutionMode, AIApplySafetyLevel } from "@/lib/ai/types";
...
export interface AIRunTracePanelProps {
  error?: string | null;
  isLoading?: boolean;
  locale?: string;
  isReplaying?: boolean;
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="grid gap-2">
          <div className="flex flex-wrap gap-2">
            ...
            {trace.source.replayLabel ? <Badge variant="warning">{trace.source.replayLabel}</Badge> : null}
            ...
            <div className="mt-4 rounded-[12px] border border-[var(--line)] bg-[color:var(--surface-panel-strong)] p-3">
              ...
              <div className="mt-2 grid gap-2">
                {trace.collaboration.consensus.map((item) => (
                  <div
                    key={item}
                    className="rounded-[10px] bg-[var(--panel-soft)] px-3 py-2 text-sm text-[var(--ink-soft)]"
                  >
                    {item}
                  </div>
                ))}
      ...
      <div className="grid gap-2">
        {trace.steps.map((step) => (
          ...
          >
            ...
            <div className="text-sm text-[var(--ink-soft)]">{step.summary}</div>
            <div className="text-xs text-[var(--ink-muted)]">
              {formatDateTime(step.startedAt, locale)} → {formatDateTime(step.endedAt, locale)}
            </div>
          </div>
        ))}
      </div>
      ...
      {trace.apply ? (
        <div className="rounded-[14px] border border-emerald-300/40 bg-emerald-500/10 p-3">
          ...
          <div className="mt-1 text-xs text-[var(--ink-muted)]">
            Applied: {formatDateTime(trace.apply.appliedAt, locale)} · Items: {trace.apply.itemCount}
          </div>
          <div className="mt-3 grid gap-2 text-xs text-[var(--ink-soft)]">
