...
test.describe("Центр загрузок", () => {
  test("shows install paths for web, desktop, and iPhone", async ({ page }) => {
    ...
    await expect(page.getByText(/живой url|локальный предпросмотр/i)).toBeVisible();
    await expect(page.getByText(/ссылка на загрузку готова|нужна ссылка на загрузку/i)).toBeVisible();
    await expect(page.getByText(/установка готова|нужна ссылка на testflight/i)).toBeVisible();
