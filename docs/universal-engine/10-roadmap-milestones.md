...
| **M2** | Stores & registries + Memory read-path | `runtime/universal/memory/{concept-store,strategy-store,memory-facade}.ts`; `runtime/universal/tool-registry.ts`; `guardrails.ts` (sandbox tier) | unit-тесты pass; `ConceptStore` project/cross-concept scope works; `StrategyStore` approved-only project isolation works; `UniversalMemoryFacade` returns approved-only non-legacy lessons; sandbox tier enforced |
