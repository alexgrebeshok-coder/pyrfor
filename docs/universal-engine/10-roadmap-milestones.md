...
|---|---|---|---|
| **M1** | Substrate of meaning + enforcement ownership | `runtime/universal/{completion-gate-engine,decision-record-auditor,legacy-node-auditor,historian}.ts`; `runtime/universal/memory/{provider,types,algorithm-aware-retriever,strategy-memory-provider,context-engine}.ts`; расширения `event-ledger.ts`, `artifact-model.ts`, `durable-dag.ts` | Implemented: completion gates block missing artifacts before `dag.node.completed`, DecisionRecord scorer is deterministic, memory v2 contracts exported, legacy baseline manifest materialization is tested, and `ue-governance-baseline-m1` anchors to pre-M1 commit `084ae02` |
...
| **M12** | Tester + AcceptanceVerification | `runtime/universal/tester.ts`; интеграция с critic | Implemented: deterministic `AcceptanceTester` runs executable `VerifyCheck` suites through `verify-engine`, emits `test.completed`, writes `test_result` acceptance reports, supports critic quorum overlay, and bounds rework cycles |
