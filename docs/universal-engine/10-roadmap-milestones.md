...
| **M11** | ToolForge + SelfExtension | `runtime/universal/{tool-forge,self-extension-loop}.ts` | Implemented: safe manifest-first ToolForge skeleton with TOC artifact gate, reuse/adapt/forge decision, mandatory static-analysis and dynamic-test evidence, network/privileged-sandbox validation, ToolForge lesson document, sandbox-only registration as `sandboxed_experiment`, regression eviction helper, and `SelfExtensionLoop` facade |
