...
| **M6** | Planner + Researcher | `runtime/universal/{planner,researcher}.ts`; `ai/orchestration/{planner,universal-planner}.ts` | plan idempotency; injection-scan before planning; bounded lookahead guards (`maxBranches`, `maxDepth`, `maxBacktracks`, `requiresNewEvidence`); OODA research loop bounded; offline-fallback Researcher; no ToolForge/gateway/orchestrator wiring |
