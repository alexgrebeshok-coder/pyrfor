...
import { sendTelegramMorningBrief } from "@/lib/retention";
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const result = await sendTelegramMorningBrief({ now: new Date() });
    return NextResponse.json(result);
  } catch (error) {
    return serverError(error, "Failed to send the Telegram morning brief.");
  }
}
