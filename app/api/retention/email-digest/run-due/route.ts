...
import { runWeeklyDigestEmails } from "@/lib/retention";
...
export async function POST(request: NextRequest): Promise<NextResponse> {
  try {
    ...
    const result = await runWeeklyDigestEmails({ now: new Date() });
    return NextResponse.json(result);
  } catch (error) {
    return serverError(error, "Failed to run scheduled email digests.");
  }
}
