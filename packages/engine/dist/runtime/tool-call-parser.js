 ...
 */
function extractNameAndArgs(obj) {
    var _a, _b, _c, _d;
    ...
    const argsSrc = (_d = (_c = (_b = (_a = obj.args) !== null && _a !== void 0 ? _a : obj.arguments) !== null && _b !== void 0 ? _b : obj.input) !== null && _c !== void 0 ? _c : obj.parameters) !== null && _d !== void 0 ? _d : {};
    const args = argsSrc && typeof argsSrc === 'object' ? argsSrc : {};
    if (!name)
        return null;
    return { name, args };
}
...
function strategyTagged(text, opts) {
    var _a;
    const calls = [];
    const coveredRanges = [];
    for (const span of locateToolCallSpans(text)) {
        ...
        if (parsed) {
            ...
            (_a = opts.onParseFailure) === null || _a === void 0 ? void 0 : _a.call(opts, {
                strategy: 'tagged',
 ...
 */
function strategyArgXml(text, opts) {
    const calls = [];
    const coveredRanges = [];
    ...
    for (const span of spans) {
        ...
        const match = body.match(/^\s*([A-Za-z_][\w-]*)\s*((?:<arg_key>[\s\S]*?<\/arg_key>\s*<arg_value>[\s\S]*?<\/arg_value>\s*)+)/);
        if (match) {
            const name = match[1];
    ...
    const standaloneRe = /([A-Za-z_][\w-]*)\s*((?:<arg_key>[\s\S]*?<\/arg_key>\s*<arg_value>[\s\S]*?<\/arg_value>\s*)+)/g;
    let standaloneMatch;
    while ((standaloneMatch = standaloneRe.exec(text)) !== null) {
        ...
        if (coveredRanges.some(([s, e]) => start >= s && end <= e))
            continue;
...
function strategyFunctionCallTag(text, opts) {
    var _a;
    const calls = [];
    const coveredRanges = [];
    ...
    while ((m = FUNCTION_CALL_TAG_RE.exec(text)) !== null) {
        ...
        if (result.success && result.data) {
            ...
            if (parsed) {
                calls.push(Object.assign(Object.assign({}, parsed), { raw: text.slice(tagStart, spanEnd) }));
...
function strategyOpenAINative(text, opts) {
    const calls = [];
    const coveredRanges = [];
    ...
    for (const item of data.tool_calls) {
        ...
        if (tc.type !== 'function')
            continue;
        if (typeof tc.function !== 'object' || !tc.function)
            continue;
        ...
        if (!argsResult.success || !argsResult.data)
            continue;
        const args = argsResult.data && typeof argsResult.data === 'object'
            ? argsResult.data
            : {};
        if (name) {
...
function strategyBareObject(text, opts) {
    const calls = [];
    const coveredRanges = [];
    ...
    while ((fenceMatch = fenceRe.exec(text)) !== null) {
        ...
        if (result.success && result.data) {
            ...
            if (parsed) {
                ...
                calls.push(Object.assign(Object.assign({}, parsed), { raw: fenceMatch[0] }));
                coveredRanges.push([start, end]);
            }
        }
    }
    ...
    while (searchStart < text.length) {
        ...
        if (result.success && result.data) {
            ...
            if (parsed) {
                ...
                const end = start + objMatch.length;
                calls.push(Object.assign(Object.assign({}, parsed), { raw: objMatch }));
                coveredRanges.push([start, end]);
                searchStart = end;
                continue;
            }
...
function strategyLineKV(text, opts) {
    const calls = [];
    const coveredRanges = [];
...
export function parseToolCalls(text, opts = {}) {
    const onParseFailure = opts.onParseFailure ||
        ((info) => {
    ...
    if (!disabled.has('tagged')) {
        const { calls, coveredRanges } = strategyTagged(text, Object.assign(Object.assign({}, opts), { onParseFailure }));
        allCalls.push(...calls);
        allCoveredRanges.push(...coveredRanges);
    }
    // Strategy 2: arg-xml.
    if (!disabled.has('arg-xml')) {
        const { calls, coveredRanges } = strategyArgXml(text, Object.assign(Object.assign({}, opts), { onParseFailure }));
        allCalls.push(...calls);
        allCoveredRanges.push(...coveredRanges);
    }
    // Strategy 3: <function_call> and <tool_use>.
    if (!disabled.has('function-call-tag')) {
        const { calls, coveredRanges } = strategyFunctionCallTag(text, Object.assign(Object.assign({}, opts), { onParseFailure }));
        allCalls.push(...calls);
        allCoveredRanges.push(...coveredRanges);
    }
    // Strategy 4: OpenAI native.
    if (!disabled.has('openai-native')) {
        const { calls, coveredRanges } = strategyOpenAINative(text, Object.assign(Object.assign({}, opts), { onParseFailure }));
        allCalls.push(...calls);
        allCoveredRanges.push(...coveredRanges);
    }
    // Strategy 5: Bare object (only if no calls found yet).
    if (!disabled.has('bare-object') && allCalls.length === 0) {
        const { calls, coveredRanges } = strategyBareObject(text, Object.assign(Object.assign({}, opts), { onParseFailure }));
        allCalls.push(...calls);
        allCoveredRanges.push(...coveredRanges);
    }
    // Strategy 6: Line key:value (only if no calls found yet).
    if (!disabled.has('line-kv') && allCalls.length === 0) {
        const { calls, coveredRanges } = strategyLineKV(text, Object.assign(Object.assign({}, opts), { onParseFailure }));
        allCalls.push(...calls);
        allCoveredRanges.push(...coveredRanges);
    }
...
export function stripToolCalls(text) {
    ...
    const argXmlRe = /([A-Za-z_][\w-]*)\s*((?:<arg_key>[\s\S]*?<\/arg_key>\s*<arg_value>[\s\S]*?<\/arg_value>\s*)+)/g;
    let argXmlMatch;
