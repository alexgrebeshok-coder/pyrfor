...
function posToLC(lineStarts, pos) {
    ...
    while (pos < source.length) {
        ...
        if (ch === '{' && (ch2 === '{' || ch2 === '%' || ch2 === '#')) {
            ...
            for (let j = innerStart; j < source.length - 1; j++) {
                if (source[j] === closeA && source[j + 1] === closeB) {
                    found = j;
                    break;
                }
            }
...
function applyWsControl(tokens) {
    const out = tokens.map(t => (Object.assign({}, t)));
    for (let i = 0; i < out.length; i++) {
        const t = out[i];
        if (t.type === 'block' || t.type === 'var') {
            if (t.stripBefore && i > 0 && out[i - 1].type === 'text') {
                out[i - 1] = Object.assign(Object.assign({}, out[i - 1]), { value: out[i - 1].value.trimEnd() });
            }
            if (t.stripAfter && i + 1 < out.length && out[i + 1].type === 'text') {
                out[i + 1] = Object.assign(Object.assign({}, out[i + 1]), { value: out[i + 1].value.trimStart() });
            }
        }
    }
    return out;
...
function parseCond(expr, line, col) {
    const s = expr.trim();
    // Boolean literals
    if (s === 'true')
...
function parse(tokens) {
    ...
    function expectKeyword(kw) {
        ...
        if (!t || t.type !== 'block' || t.value.split(/\s+/)[0] !== kw) {
            ...
            throw new TemplateError(`Expected '{% ${kw} %}' but got ${got}`, (_a = t === null || t === void 0 ? void 0 : t.line) !== null && _a !== void 0 ? _a : 1, (_b = t === null || t === void 0 ? void 0 : t.col) !== null && _b !== void 0 ? _b : 1);
        }
        return tokens[pos++];
    }
    function parseBody(stopAt) {
        const nodes = [];
        while (pos < tokens.length) {
            const t = cur();
            if (t.type === 'block' && stopAt.includes(t.value.split(/\s+/)[0]))
                break;
    ...
    function parseNode() {
        const t = cur();
        if (!t)
            throw new TemplateError('Unexpected end of template', 1, 1);
        if (t.type === 'text') {
            pos++;
    ...
    function parseFor() {
        ...
        if (!m)
            throw new TemplateError(`Invalid for syntax: expected 'for item in list'`, ft.line, ft.col);
        const item = m[1];
    ...
    function parseInclude() {
        ...
        if (!m)
            throw new TemplateError(`Invalid include syntax: expected include 'name'`, t.line, t.col);
...
// ─── Built-in filters ─────────────────────────────────────────────────────────
const BUILTIN_FILTERS = new Map([
    ['upper', (v) => v == null ? '' : String(v).toUpperCase()],
    ['lower', (v) => v == null ? '' : String(v).toLowerCase()],
    ['trim', (v) => v == null ? '' : String(v).trim()],
    ['json', (v) => { var _a; return v === undefined ? 'null' : ((_a = JSON.stringify(v)) !== null && _a !== void 0 ? _a : 'null'); }],
    ['length', (v) => v == null ? 0 : typeof v === 'string' || Array.isArray(v) ? v.length : typeof v === 'object' ? Object.keys(v).length : 0],
    ['default', (v, fb = '') => v == null ? fb : v],
]);
// ─── Hash (djb2) ──────────────────────────────────────────────────────────────
function hashStr(s) {
    let h = 5381;
...
export function createPromptTemplateEngine(opts) {
    var _a;
    const maxCache = (_a = opts === null || opts === void 0 ? void 0 : opts.cacheSize) !== null && _a !== void 0 ? _a : 64;
    const log = opts === null || opts === void 0 ? void 0 : opts.logger;
    const partials = new Map();
    const customFilters = new Map();
    const cache = new LRUCache(maxCache);
    // ── compile ──────────────────────────────────────────────────────────────
    function compile(source) {
        const key = hashStr(source);
        if (cache.has(key)) {
            log === null || log === void 0 ? void 0 : log('cache:hit', { key });
            return cache.get(key);
        }
        log === null || log === void 0 ? void 0 : log('cache:miss', { key });
        const ast = parse(applyWsControl(lex(source)));
        const fn = (ctx, stack) => evalAst(ast, ctx, stack);
        cache.set(key, fn);
        return fn;
    }
    // ── evaluator ────────────────────────────────────────────────────────────
    function evalAst(nodes, ctx, stack) {
        let out = '';
        for (const n of nodes)
            out += evalNode(n, ctx, stack);
        return out;
    }
    function evalNode(node, ctx, stack) {
        switch (node.type) {
            case 'text':
                return node.value;
            case 'var': {
                let val = resolvePath(node.path, ctx);
            ...
            case 'if': {
                for (const branch of node.branches) {
                    if (evalCond(branch.cond, ctx))
                        return evalAst(branch.body, ctx, stack);
                }
            ...
            case 'for': {
                ...
                for (let i = 0; i < list.length; i++) {
                    const loopCtx = Object.assign(Object.assign({}, ctx), { [node.item]: list[i], loop: { index0: i, index1: i + 1 } });
            ...
            case 'include': {
                ...
                if (stack.includes(node.name)) {
                    throw new TemplateError(`Circular partial reference: ${[...stack, node.name].join(' → ')}`, node.line, node.col);
                }
    ...
    function applyFilter(f, val) {
        var _a;
        const fn = (_a = customFilters.get(f.name)) !== null && _a !== void 0 ? _a : BUILTIN_FILTERS.get(f.name);
        if (!fn)
        ...
        switch (cond.kind) {
            ...
            case 'cmp': {
                ...
                const rv = (typeof cond.right === 'object' && cond.right !== null && '_path' in cond.right)
                    ? resolvePath(cond.right._path, ctx)
                    : cond.right;
                switch (cond.op) {
    ...
    return {
        ...
        renderTemplate(name, ctx) {
            ...
            if (src === undefined)
                throw new TemplateError(`Template '${name}' not found`, 1, 1);
