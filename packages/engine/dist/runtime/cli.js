 ...
 */
async function runDaemon(runtime) {
    logger_1.logger.info('Running in daemon mode');
 ...
 */
async function runChat(runtime, options) {
    logger_1.logger.info('Running in interactive chat mode');
 ...
 * Telegram bot mode
 */
async function runTelegram(runtime) {
    logger_1.logger.info('Running in Telegram bot mode');
    const token = process.env.TELEGRAM_BOT_TOKEN;
    if (!token) {
        logger_1.logger.error('TELEGRAM_BOT_TOKEN not set');
        // eslint-disable-next-line no-console
    ...
    try {
        TelegramBotModule = await Promise.resolve().then(() => __importStar(require('node-telegram-bot-api')));
    }
    catch {
        logger_1.logger.error('node-telegram-bot-api not installed');
        // eslint-disable-next-line no-console
...
Total cost: $${stats.providers.costs.totalUsd.toFixed(4)}`;
    ...
    bot.on('polling_error', (error) => {
        logger_1.logger.error('Telegram polling error', { error: String(error) });
    });
    // Keep alive
    process.on('SIGINT', async () => {
        logger_1.logger.info('Shutting down Telegram bot...');
        bot.stopPolling();
        await runtime.stop();
        process.exit(0);
    });
 ...
 * One-shot mode
 */
async function runOnce(runtime, options) {
    ...
    logger_1.logger.info('Running in one-shot mode', { message: message.slice(0, 100) });
...
async function main() {
    ...
    const runtime = new index_1.PyrforRuntime({
    ...
    await runtime.start();
    const stats = runtime.getStats();
    logger_1.logger.info('Runtime started', {
...
// Run main
main().catch((error) => {
    logger_1.logger.error('Fatal error', { error: String(error) });
    // eslint-disable-next-line no-console
