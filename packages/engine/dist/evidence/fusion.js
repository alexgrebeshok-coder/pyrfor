...
import { getEvidenceLedgerOverview } from './service.js';
...
export function getEvidenceFusionOverview() {
    return __awaiter(this, arguments, void 0, function* (query = {}, deps = {}) {
        var _a, _b, _c, _d;
        const now = (_a = deps.now) !== null && _a !== void 0 ? _a : (() => new Date());
        const getEvidence = (_b = deps.getEvidence) !== null && _b !== void 0 ? _b : getEvidenceLedgerOverview;
        const evidence = (_c = deps.evidence) !== null && _c !== void 0 ? _c : (yield getEvidence(Object.assign({ limit: deriveLedgerLimit(query.limit) }, (query.projectId ? { projectId: query.projectId } : {}))));
        ...
        return {
            syncedAt: (_d = evidence.syncedAt) !== null && _d !== void 0 ? _d : now().toISOString(),
            summary: summarizeFusionFacts(facts),
            facts,
        };
    });
}
export function buildEvidenceFusionFacts(records) {
...
function buildFusionFact(report, videoFacts, gpsSessions) {
    var _a, _b;
    ...
    const reportNumber = (_b = (_a = readMetadataString(reportMetadata, "reportNumber")) !== null && _a !== void 0 ? _a : report.sourceRef) !== null && _b !== void 0 ? _b : report.entityRef;
    ...
    const observedAt = sources.reduce((latest, source) => (Date.parse(source.observedAt) > Date.parse(latest) ? source.observedAt : latest), report.observedAt);
    return {
...
function matchVideoFactToReport(report, record) {
    var _a;
    ...
    const videoDate = (_a = readMetadataString(record.metadata, "reportDate")) !== null && _a !== void 0 ? _a : record.observedAt;
    if (isSameUtcDay(reportDate !== null && reportDate !== void 0 ? reportDate : report.observedAt, videoDate)) {
...
function matchGpsSessionToReport(report, record) {
    const reportDate = readMetadataString(report.metadata, "reportDate");
    if (!isSameUtcDay(reportDate !== null && reportDate !== void 0 ? reportDate : report.observedAt, record.observedAt)) {
        return null;
    }
...
function deriveFusionStatus(report, supports) {
...
function mergeConfidence(report, supports) {
...
function toFusionSource(record, matchReasons) {
    return {
        recordId: record.id,
...
function buildExplanation(report, supports, verificationStatus, confidence) {
    var _a, _b;
    const reportNumber = (_b = (_a = readMetadataString(report.metadata, "reportNumber")) !== null && _a !== void 0 ? _a : report.sourceRef) !== null && _b !== void 0 ? _b : report.entityRef;
    ...
    const sourceLabels = Array.from(new Set(supports.map((support) => support.record.entityType === "video_fact" ? "visual evidence" : "GPS telemetry")));
...
function summarizeFusionFacts(facts) {
    var _a, _b;
    if (facts.length === 0) {
        return {
            total: 0,
    ...
    const summary = facts.reduce((accumulator, fact) => {
        accumulator.total += 1;
    ...
    return {
        ...
        strongestFactTitle: (_b = (_a = summary.strongestFact) === null || _a === void 0 ? void 0 : _a.title) !== null && _b !== void 0 ? _b : null,
    };
}
function deriveLedgerLimit(limit) {
...
function sanitizeLimit(limit) {
    ...
    return Math.min(Math.max(Math.trunc(limit !== null && limit !== void 0 ? limit : 6), 1), 12);
}
