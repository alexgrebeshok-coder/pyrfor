/** OpenTelemetry GenAI semantic convention attribute keys used by the engine. */
export declare const GEN_AI_OPERATION_NAME = "gen_ai.operation.name";
export declare const GEN_AI_REQUEST_MODEL = "gen_ai.request.model";
export declare const GEN_AI_USAGE_INPUT_TOKENS = "gen_ai.usage.input_tokens";
export declare const GEN_AI_USAGE_OUTPUT_TOKENS = "gen_ai.usage.output_tokens";
export declare const GEN_AI_USAGE_TOTAL_TOKENS = "gen_ai.usage.total_tokens";
export declare const GEN_AI_USAGE_COST = "gen_ai.usage.cost";
export declare const GEN_AI_AGENT_STEP = "gen_ai.agent.step";
export declare const GEN_AI_TOOL_NAME = "gen_ai.tool.name";
export type GenAiAgentStep = 'plan' | 'research' | 'execute' | 'critique' | 'postmortem' | 'memory_persist';
export declare function genAiLifecycleAttrs(step: GenAiAgentStep): Record<string, string>;
//# sourceMappingURL=genai-attrs.d.ts.map