...
export function isDemoWorkspacePath(pathname) {
    if (!pathname) {
        return false;
    }
    return (pathname === DEMO_WORKSPACE_BASE_PATH ||
        pathname.startsWith(`${DEMO_WORKSPACE_BASE_PATH}/`));
}
export function stripDemoWorkspacePrefix(pathname) {
    if (!pathname) {
        return "/";
    }
...
export function toDemoWorkspaceHref(href, currentPathname) {
    if (!href.startsWith("/")) {
        return href;
    }
