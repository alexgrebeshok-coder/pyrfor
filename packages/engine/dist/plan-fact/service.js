export function buildProjectPlanFactSummary(snapshot, projectId, options = {}) {
    var _a;
    ...
    const overdueMilestones = milestones.filter((milestone) => milestone.status !== "completed" &&
        new Date(milestone.date).getTime() < referenceDate.getTime()).length;
    ...
    const progressVarianceRatio = plannedProgress > 0 ? round(progressVariance / plannedProgress, 3) : 0;
    ...
    const budgetVarianceRatio = budgetPlanToDate > 0 ? round(budgetVariance / budgetPlanToDate, 3) : 0;
...
export function buildPortfolioPlanFactSummary(snapshot, options = {}) {
    var _a;
    const referenceDate = toDate((_a = options.referenceDate) !== null && _a !== void 0 ? _a : snapshot.generatedAt);
    const projects = snapshot.projects.map((project) => buildProjectPlanFactSummary(snapshot, project.id, { referenceDate }));
    ...
    const topSignals = projects
        .flatMap((project) => project.warnings.map((warning) => (Object.assign(Object.assign({}, warning), { projectId: project.projectId, projectName: project.projectName }))))
        .sort(compareSignals)
        .slice(0, 8);
    return {
        referenceDate: referenceDate.toISOString(),
        ...
        totals: {
            ...
            staleFieldReportingProjects: projects.filter((project) => project.evidence.daysSinceLastApprovedReport !== null &&
                project.evidence.daysSinceLastApprovedReport >= 3).length,
            pendingReviewProjects: projects.filter((project) => project.evidence.pendingWorkReports >= 2).length,
...
export function summarizeProjectPlanFactForBrief(summary) {
    return {
...
function calculatePlannedProgress(project, referenceDate) {
...
function calculateBudgetPlanToDate(project, referenceDate) {
    ...
    if (datedHistory.length > 0) {
        return round((_b = (_a = datedHistory[datedHistory.length - 1]) === null || _a === void 0 ? void 0 : _a.budgetPlanned) !== null && _b !== void 0 ? _b : 0, 2);
    }
    return round(project.budget.planned * (calculatePlannedProgress(project, referenceDate) / 100), 2);
}
function calculateActualProgress(project, taskProgress, milestoneProgress) {
    ...
    return round(weightedSignals.reduce((accumulator, signal) => accumulator + signal.value * signal.weight, 0) /
        totalWeight, 1);
}
function calculateEvmMetrics(project, plannedProgress, actualProgress) {
...
function latestApprovedReportDate(workReports) {
...
function calculateConfidence(input, referenceDate) {
    let confidence = 0.35;
    if (input.taskCount > 0)
        confidence += 0.25;
    if (input.milestoneCount > 0)
        confidence += 0.15;
    if (input.totalWorkReports > 0)
        confidence += 0.15;
...
function buildWarnings(input) {
    ...
    const daysToMilestone = nextMilestone
        ...
        : null;
    if (input.progressVariance <= -10 ||
        input.overdueTasks > 0 ||
        (input.evm.spi !== null && input.evm.spi < 0.9)) {
        warnings.push({
            code: "SCHEDULE_DRIFT",
            severity: diffInDays(input.referenceDate, new Date(input.project.dates.end)) < 0 ||
    ...
    if (input.evm.ac > 0 &&
        ...
        const overspending = input.budgetVarianceRatio >= 0.1 || input.evm.ac > input.evm.bac;
        const lowEfficiency = input.evm.cpi !== null && input.evm.cpi < 0.9;
        warnings.push({
            code: "COST_PRESSURE",
            severity: input.budgetVarianceRatio >= 0.2 ||
                input.evm.ac > input.evm.bac ||
    ...
    if (input.overdueMilestones > 0 ||
        (daysToMilestone !== null &&
            daysToMilestone <= 14 &&
            ((_b = (_a = input.taskProgress) !== null && _a !== void 0 ? _a : input.milestoneProgress) !== null && _b !== void 0 ? _b : input.project.progress) + 8 <
                input.plannedProgress)) {
        warnings.push({
            code: "MILESTONE_RISK",
            severity: input.overdueMilestones > 0 || (daysToMilestone !== null && daysToMilestone <= 7)
                ? "high"
                : "medium",
...
function deriveProjectStatus(warnings) {
...
function forecastFinishDate(project, actualProgress, spi, referenceDate) {
    ...
    const baseRemaining = plannedEnd > reference ? plannedEnd - reference : totalDuration * (1 - actualProgress / 100);
