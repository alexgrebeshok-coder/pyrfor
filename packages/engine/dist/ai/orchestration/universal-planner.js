...
export class ModelCapViolationError extends Error {
    constructor(modelId) {
        super(`universal-planner: model "${modelId}" exceeds M6 cap. ` +
            `Allowed: ${[...M6_ALLOWED_MODELS].join(', ')}`);
        this.name = 'ModelCapViolationError';
    }
}
export function assertM6ModelCap(modelId) {
    if (!M6_ALLOWED_MODELS.has(modelId))
        throw new ModelCapViolationError(modelId);
}
...
// ─── Heuristic Analysis ──────────────────────────────────────────────────────
const RESEARCH_RE = /\b(research|investigate|find\s+out|study|analyze|analyse|learn\s+about|discover|explore|survey)\b/i;
const COMPLEXITY_RE = /\b(integrate|build|implement|create|design|architect|develop|deploy|migrate|refactor|orchestrate)\b/i;
const RESEARCH_TOPIC_RE = /(?:research|investigate|find(?:\s+out)?|study|analyze|learn(?:\s+about)?)\s+(?:the\s+|a\s+)?([^,.;]{3,60})/i;
const LOOKAHEAD_HARD_CAPS = Object.freeze({
    maxBranches: 5,
    maxDepth: 8,
    maxBacktracks: 3,
});
export function evaluateLookaheadBounds(config, usage) {
    const effectiveLimits = {
        maxBranches: clampLimit(config === null || config === void 0 ? void 0 : config.maxBranches, LOOKAHEAD_HARD_CAPS.maxBranches),
        maxDepth: clampLimit(config === null || config === void 0 ? void 0 : config.maxDepth, LOOKAHEAD_HARD_CAPS.maxDepth),
        maxBacktracks: clampLimit(config === null || config === void 0 ? void 0 : config.maxBacktracks, LOOKAHEAD_HARD_CAPS.maxBacktracks),
    };
    const reasonCodes = [];
    if (usage.branches > effectiveLimits.maxBranches)
        reasonCodes.push('max_branches_exceeded');
    if (usage.depth > effectiveLimits.maxDepth)
        reasonCodes.push('max_depth_exceeded');
    if (usage.backtracks > effectiveLimits.maxBacktracks)
        reasonCodes.push('max_backtracks_exceeded');
    if ((config === null || config === void 0 ? void 0 : config.requiresNewEvidence) === true &&
...
function extractResearchTopics(concept) {
    const topics = [];
...
function buildHeuristicSteps(concept, phases) {
    return phases
        .filter((p) => p !== 'done')
        .map((phase, index) => ({
...
function clampLimit(value, hardCap) {
    if (value === undefined)
        return hardCap;
    return Math.max(0, Math.min(Math.floor(value), hardCap));
}
function phaseTitle(phase, concept) {
...
function phaseDescription(phase, _concept) {
    const map = {
...
function phaseAcceptanceCriteria(phase) {
    const map = {
...
function phaseTokenBudget(phase) {
    const map = {
        plan: 2000,
        research: 4000,
        execute: 8000,
 ...
 */
export function buildUniversalPlanHeuristic(concept, context) {
    var _a;
    if (!concept.trim())
        throw new Error('universal-planner: concept must not be empty');
    ...
    const createdAt = ((_a = context.now) !== null && _a !== void 0 ? _a : (() => new Date().toISOString()))();
    ...
    if (!lookaheadDecision.allowed)
        throw new LookaheadBoundsViolationError(lookaheadDecision);
    const planDocument = {
    ...
    const collaborationSteps = isComplex
        ? [
            {
...
const VALID_PHASES = new Set(['plan', 'research', 'execute', 'critique', 'done']);
function normalizeLLMPhases(phases) {
    if (!Array.isArray(phases))
        return ['plan', 'execute', 'critique', 'done'];
    const normalized = phases.filter((p) => typeof p === 'string' && VALID_PHASES.has(p));
    if (!normalized.includes('plan'))
        normalized.unshift('plan');
 ...
 */
export function buildUniversalPlan(concept, context, llmAdapter) {
    return __awaiter(this, void 0, void 0, function* () {
        if (!concept.trim())
            throw new Error('universal-planner: concept must not be empty');
        if (llmAdapter)
            assertM6ModelCap(llmAdapter.modelId);
        const heuristic = buildUniversalPlanHeuristic(concept, context);
        if (!llmAdapter)
            return heuristic;
        let raw;
        try {
            raw = yield llmAdapter.complete(PLAN_SYSTEM_PROMPT, buildPlanUserPrompt(concept, context));
        }
        catch (_a) {
        ...
        return Object.assign(Object.assign({}, heuristic), { planDocument,
            phases,
            researchRequired,
            idempotencyKey });
    });
