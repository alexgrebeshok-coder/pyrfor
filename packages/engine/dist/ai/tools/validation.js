...
function isValueCompatible(value, expectedType) {
...
export function validateToolParameters(schema, params) {
    var _a, _b;
    ...
    const requiredKeys = (_a = schema.required) !== null && _a !== void 0 ? _a : [];
    ...
    const shape = (_b = schema.properties) !== null && _b !== void 0 ? _b : {};
    for (const [key, propertySchema] of Object.entries(shape)) {
        ...
        if (propertySchema.enum &&
            ...
            return `Parameter '${key}' must be one of: ${formatEnum(propertySchema.enum)}`;
        }
    }
    return null;
}
export function formatToolResultForContext(result) {
    try {
        ...
        return `Unable to render tool result: ${error instanceof Error ? error.message : String(error)}`;
    }
}
