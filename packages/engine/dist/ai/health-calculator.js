 ...
 */
export function calculatePortfolioHealth(projects, risks, teamUtilization) {
    ...
    const overall = Math.round(budgetHealth * 0.3 + scheduleHealth * 0.3 + riskHealth * 0.25 + resourceHealth * 0.15);
    return {
        overall,
 ...
 */
function calculateProjectEVM(project) {
 ...
 */
function calculateCPIScore(cpiValues) {
    ...
    if (avgCPI >= 1.1)
        return 100;
    if (avgCPI >= 1.0)
        return 70 + (avgCPI - 1.0) * 300; // 70-100
    if (avgCPI >= 0.9)
        return 40 + (avgCPI - 0.9) * 300; // 40-70
    return Math.max(0, avgCPI * 44.44); // 0-40
}
/**
 * Convert SPI values to health score (0-100)
 */
function calculateSPIScore(spiValues) {
    ...
    if (avgSPI >= 1.1)
        return 100;
    if (avgSPI >= 1.0)
        return 70 + (avgSPI - 1.0) * 300; // 70-100
    if (avgSPI >= 0.9)
        return 40 + (avgSPI - 0.9) * 300; // 40-70
    return Math.max(0, avgSPI * 44.44); // 0-40
}
/**
 * Calculate resource health based on team utilization
 */
function calculateResourceHealth(utilization) {
    if (utilization <= 80)
        return 100;
    if (utilization <= 100)
        return 100 - ((utilization - 80) / 20) * 50; // 100-50
