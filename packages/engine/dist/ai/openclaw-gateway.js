...
function normalizeGatewayUrl(input) {
    const raw = (input === null || input === void 0 ? void 0 : input.trim()) || DEFAULT_GATEWAY_URL;
    try {
...
function stripModelMarkers(text) {
...
function fetchWithRetry(url_1, init_1) {
    return __awaiter(this, arguments, void 0, function* (url, init, retries = 3) {
        var _a;
        let lastError = null;
        for (let attempt = 0; attempt < retries; attempt += 1) {
            ...
            try {
                ...
                lastError = new Error(responseText.trim().length
        ...
        throw lastError !== null && lastError !== void 0 ? lastError : new Error("Gateway request failed after retries.");
    });
}
function consumeSseText(response) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const dispatch = () => {
            ...
            const payloadType = typeof (payload === null || payload === void 0 ? void 0 : payload.type) === "string" ? payload.type.toLowerCase() : eventType.toLowerCase();
            const payloadStatus = typeof (payload === null || payload === void 0 ? void 0 : payload.status) === "string" ? payload.status.toLowerCase() : null;
            if (trimmed === "[DONE]") {
                terminal = true;
            }
            else if (payloadType.includes("response.output_text.delta") &&
                typeof (payload === null || payload === void 0 ? void 0 : payload.delta) === "string") {
            ...
            else if (payloadType.includes("failed") ||
                ...
                failed = true;
                errorMessage =
                    (typeof (payload === null || payload === void 0 ? void 0 : payload.error) === "string" && payload.error) ||
                        (typeof (payload === null || payload === void 0 ? void 0 : payload.message) === "string" && payload.message) ||
                        trimmed ||
                        "Gateway SSE stream failed.";
            }
            else if (payloadType.includes("completed") ||
        ...
        if (failed) {
            throw new Error(errorMessage !== null && errorMessage !== void 0 ? errorMessage : "Gateway SSE stream failed.");
        }
...
function buildContextDigest(input) {
    ...
    const relevantTasks = (_a = (project ? context.projectTasks : context.tasks)) === null || _a === void 0 ? void 0 : _a.slice(0, 10).map((task) => {
        var _a;
        return ({
            id: task.id,
            ...
            blockedReason: (_a = task.blockedReason) !== null && _a !== void 0 ? _a : null,
        });
    });
    ...
    return {
        ...
        quickAction: input.quickAction
            ...
            : null,
        source: (_b = input.source) !== null && _b !== void 0 ? _b : null,
        ...
        tasks: relevantTasks !== null && relevantTasks !== void 0 ? relevantTasks : [],
        risks: relevantRisks,
...
export function buildGatewayPrompt(input, runId) {
    const localeLabel = input.context.locale === "zh"
        ? "Simplified Chinese"
...
function buildProposal(value, runId) {
    var _a, _b, _c, _d, _e, _f;
    if (!value) {
        return null;
    }
    const inferredType = (_a = normalizeActionType(value.type)) !== null && _a !== void 0 ? _a : (Array.isArray(value.tasks) && value.tasks.length
        ? "create_tasks"
    ...
    const title = typeof value.title === "string" && value.title.trim().length
        ? value.title.trim()
        : "AI proposal";
    const summary = typeof value.summary === "string" && value.summary.trim().length
        ? value.summary.trim()
        : "AI action prepared by the gateway.";
    switch (inferredType) {
        case "create_tasks": {
            const tasks = ((_b = value.tasks) !== null && _b !== void 0 ? _b : [])
                .map((task, index) => ({
                projectId: typeof task.projectId === "string" ? task.projectId : "",
                title: typeof task.title === "string" ? task.title.trim() : "",
                description: typeof task.description === "string" ? task.description.trim() : "AI-generated task",
        ...
        case "update_tasks": {
            const taskUpdates = ((_c = value.taskUpdates) !== null && _c !== void 0 ? _c : [])
                .map((task, index) => ({
                taskId: typeof task.taskId === "string" ? task.taskId : "",
                title: typeof task.title === "string" ? task.title.trim() : "",
                description: typeof task.description === "string" ? task.description.trim() : undefined,
                assignee: typeof task.assignee === "string" && task.assignee.trim().length
                    ? task.assignee.trim()
                    : undefined,
        ...
        case "reschedule_tasks": {
            const taskReschedules = ((_d = value.taskReschedules) !== null && _d !== void 0 ? _d : [])
                .map((task, index) => ({
                taskId: typeof task.taskId === "string" ? task.taskId : "",
                title: typeof task.title === "string" ? task.title.trim() : "",
                previousDueDate: typeof task.previousDueDate === "string" && /^\d{4}-\d{2}-\d{2}$/.test(task.previousDueDate)
        ...
        case "raise_risks": {
            const risks = ((_e = value.risks) !== null && _e !== void 0 ? _e : [])
                .map((risk) => ({
                projectId: typeof risk.projectId === "string" ? risk.projectId : "",
                title: typeof risk.title === "string" ? risk.title.trim() : "",
                description: typeof risk.description === "string" ? risk.description.trim() : "AI-generated risk",
                owner: typeof risk.owner === "string" ? risk.owner.trim() : "Owner",
                probability: typeof risk.probability === "number" ? Math.max(0, Math.min(100, risk.probability)) : 60,
                impact: typeof risk.impact === "number" ? Math.max(0, Math.min(100, risk.impact)) : 60,
                mitigation: typeof risk.mitigation === "string" ? risk.mitigation.trim() : "Mitigation pending.",
        ...
        case "draft_status_report": {
            ...
            const projectId = typeof report.projectId === "string" && report.projectId.trim().length
                ? report.projectId.trim()
                : undefined;
            const reportTitle = typeof report.title === "string" && report.title.trim().length
                ? report.title.trim()
                : "Status draft";
            const audience = typeof report.audience === "string" && report.audience.trim().length
                ? report.audience.trim()
                : "Stakeholders";
            const channel = typeof report.channel === "string" && report.channel.trim().length
                ? report.channel.trim()
                : "weekly update";
            const reportSummary = typeof report.summary === "string" && report.summary.trim().length
                ? report.summary.trim()
                : summary;
            const body = typeof report.body === "string" && report.body.trim().length
                ? report.body.trim()
                : reportSummary;
            return {
                id: `proposal-${runId}`,
                ...
                statusReport: {
                    ...
                    body,
                    reason: typeof report.reason === "string" && report.reason.trim().length
                        ? report.reason.trim()
                        : "AI suggestion",
                },
            };
        ...
        case "notify_team": {
            const notifications = ((_f = value.notifications) !== null && _f !== void 0 ? _f : [])
                .map((item) => ({
                channel: typeof item.channel === "string" && item.channel.trim().length
                    ? item.channel.trim()
                    : "team-ops",
                recipients: Array.isArray(item.recipients)
                    ? item.recipients
                        .filter((recipient) => typeof recipient === "string" && recipient.trim().length > 0)
                        .map((recipient) => recipient.trim())
                        .slice(0, 6)
                    : [],
                message: typeof item.message === "string" && item.message.trim().length
                    ? item.message.trim()
                    : "",
                reason: typeof item.reason === "string" && item.reason.trim().length
                    ? item.reason.trim()
                    : "AI suggestion",
            }))
...
export function parseGatewayResult(rawText, runId) {
    var _a, _b, _c, _d;
    ...
    if (!parsed) {
        ...
        const fallbackLine = (_a = fallbackSummary
            .split("\n")
            .map((line) => line.trim())
            .find((line) => line.length > 0)) !== null && _a !== void 0 ? _a : "Gateway response";
        return {
    ...
    if (typeof result.title !== "string" ||
        ...
        const fallbackSummary = collectOutputText(result).join("\n").trim() || normalizedText || "Gateway returned an empty response.";
        const fallbackLine = (_b = fallbackSummary
            .split("\n")
            .map((line) => line.trim())
            .find((line) => line.length > 0)) !== null && _b !== void 0 ? _b : "Gateway response";
        return {
            ...
            proposal: buildProposal((_c = result.proposal) !== null && _c !== void 0 ? _c : null, runId),
        };
    }
    return {
        ...
        facts: normalizeChatFacts(result.facts),
        confidence: normalizeChatConfidence(result.confidence),
        proposal: buildProposal((_d = result.proposal) !== null && _d !== void 0 ? _d : null, runId),
    };
}
export function invokeOpenClawGateway(input, runId, options) {
    return __awaiter(this, void 0, void 0, function* () {
        var _a, _b, _c;
        const gatewayUrl = normalizeGatewayUrl(process.env.OPENCLAW_GATEWAY_URL);
        const token = (_a = process.env.OPENCLAW_GATEWAY_TOKEN) === null || _a === void 0 ? void 0 : _a.trim();
        const prompt = (_b = options === null || options === void 0 ? void 0 : options.promptOverride) !== null && _b !== void 0 ? _b : buildGatewayPrompt(input, runId);
        const headers = {
        ...
        const response = yield fetchWithRetry(gatewayUrl, {
            method: "POST",
            headers,
            body: JSON.stringify({
                model: (_c = process.env.OPENCLAW_GATEWAY_MODEL) !== null && _c !== void 0 ? _c : "openclaw:main",
                stream: true,
                messages: [{ role: "user", content: prompt }],
            }),
        });
        const outputText = yield consumeSseText(response);
