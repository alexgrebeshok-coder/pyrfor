 ...
 */
export function generateInsights(projects, evmMetricsMap, risksMap) {
    const insights = [];
    ...
    for (const [projectId, metrics] of evmMetricsMap.entries()) {
        ...
        const avgSpi = allSpiValues.reduce((sum, spi) => sum + spi, 0) / allSpiValues.length;
 ...
 */
function analyzeCPITrend(projects, _evmMetricsMap) {
    let totalChange = 0;
    let count = 0;
 ...
 */
function getCommonDirections(projects) {
