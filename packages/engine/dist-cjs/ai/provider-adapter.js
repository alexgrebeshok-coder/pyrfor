...
exports.hasAvailableProviders = hasAvailableProviders;
...
function loadConfiguredProviders(env = process.env) {
    const manifests = (0, provider_manifests_1.loadConfiguredAIProviderManifests)(env);
    const providers = [...BUILTIN_PROVIDERS];
    for (const manifest of manifests) {
        if (providers.some((provider) => provider.name === manifest.name)) {
            logger_1.logger.warn("Skipping duplicate AI provider manifest", {
...
function getAvailableProviders() {
    ...
    if (providers.length === 0) {
        logger_1.logger.error("No AI providers configured. Set at least one API key.", {
...
class ProviderAdapter {
    constructor(options) {
        ...
        this.mockAdapter = (0, mock_adapter_1.createMockAIAdapter)();
    }
    getDefaultPriority() {
    ...
    async runAgent(input) {
        ...
        this.executeWithProviders(runId, restInput, signal).catch((error) => {
            logger_1.logger.error("All AI providers failed", { error: error instanceof Error ? error.message : String(error) });
    ...
    async executeWithProviders(runId, input, signal) {
        const errors = [];
        ...
        for (const providerName of this.priority) {
            ...
            if (!this.isProviderAvailable(providerName)) {
                logger_1.logger.debug("Provider not available (no API key)", { provider: providerName });
                continue;
            }
            try {
                logger_1.logger.info("Trying AI provider", { provider: providerName });
                attemptedProviders.push(providerName);
                const result = await this.tryProvider(providerName, input, signal);
                logger_1.logger.info("Provider succeeded", { provider: providerName });
                ...
                logger_1.logger.warn("Provider failed", { provider: providerName, error: error instanceof Error ? error.message : String(error) });
                errors.push(error);
        ...
        logger_1.logger.warn("All providers failed, marking run failed", {
            attemptedProviders,
    ...
    async tryProvider(providerName, input, signal) {
...
Be concise, specific, and practical. Focus on actions, not descriptions.`;
    ...
    buildFinalRun(runId, input, result, _providerName) {
        ...
        return {
            ...
            updatedAt: timestamp,
            context: input.context.activeContext,
            result: (0, grounding_1.attachRunGrounding)({
                title: input.quickAction?.id || "AI Analysis",
    ...
    buildFailedRun(runId, input, run, error) {
        ...
        const message = error instanceof Error
            ...
            : typeof error === "string"
                ? error
                : error?.message ?? "AI provider run failed";
        return {
            ...run,
            id: runId,
            title: input.quickAction?.id || "AI Analysis",
