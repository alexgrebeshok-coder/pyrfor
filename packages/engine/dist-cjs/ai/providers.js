...
exports.aiRouter = exports.AIRouter = exports.YandexGPTProvider = exports.GigaChatProvider = exports.BothubProvider = exports.PolzaProvider = exports.AIJoraProvider = exports.OpenAIProvider = exports.ZAIProvider = exports.OpenRouterProvider = void 0;
exports.isTransientProviderError = isTransientProviderError;
exports.getRouter = getRouter;
exports.hasAvailableProvider = hasAvailableProvider;
exports.getProviderName = getProviderName;
require("server-only");
const logger_1 = require("../observability/logger");
const provider_manifests_1 = require("./provider-manifests");
...
class OpenRouterProvider {
    ...
    async chat(messages, options) {
        ...
        for (const model of fallbackChain) {
            ...
            let body = '';
            try {
                ({ status, body } = await this.httpsPost(JSON.stringify({
                    model,
                ...
                logger_1.logger.warn('OpenRouter network failure, trying next in-provider model', {
                    model,
            ...
            logger_1.logger.warn('OpenRouter model fallback', {
                model,
                status,
    ...
    async chatWithTools(messages, options) {
        ...
        for (const model of chain) {
            ...
            try {
                ...
                const msg = networkErr instanceof Error ? networkErr.message : String(networkErr);
                logger_1.logger.warn('OpenRouter tool-call network failure, trying next model', {
                    model,
                    reason: msg.slice(0, 160),
                });
                lastError = msg;
            ...
            logger_1.logger.warn('OpenRouter tool-call model fallback', {
                model,
                status,
...
async function openAICompatibleChatWithTools(params) {
    ...
    for (const model of chain) {
        ...
        try {
            ...
            const msg = networkErr instanceof Error ? networkErr.message : String(networkErr);
            logger_1.logger.warn(`${providerName}: tool-call network failure, trying next model`, {
                model,
                reason: msg.slice(0, 160),
            });
            lastError = msg;
        ...
        logger_1.logger.warn(`${providerName}: tool-call model fallback`, {
            model,
            status,
            reason: isRateLimit ? 'rate-limit' : 'server-error',
        });
...
exports.AIJoraProvider = AIJoraProvider;
// ============================================
...
class GigaChatProvider {
    ...
    async chatWithTools(messages, options) {
        ...
        for (const model of chain) {
            let response;
            try {
                response = await fetchWithTimeout('https://gigachat.devices.sberbank.ru/api/v1/chat/completions', {
                    method: 'POST',
                    headers: {
                        Authorization: `Bearer ${token}`,
                        'Content-Type': 'application/json',
                ...
                const msg = networkErr instanceof Error ? networkErr.message : String(networkErr);
                logger_1.logger.warn('gigachat: tool-call network failure, trying next model', {
                    model,
                    reason: msg.slice(0, 160),
                });
                lastError = msg;
            ...
            logger_1.logger.warn('gigachat: tool-call model fallback', {
                model,
                status,
                reason: isRateLimit ? 'rate-limit' : 'server-error',
            });
...
class YandexGPTProvider {
    ...
    async chatWithTools(messages, options) {
        ...
        for (const model of chain) {
            ...
            let response;
            try {
                response = await fetchWithTimeout('https://llm.api.cloud.yandex.net/foundationModels/v1/completion', {
                    method: 'POST',
                    headers: {
                        Authorization: `Api-Key ${this.apiKey}`,
                        'Content-Type': 'application/json',
                ...
                const msg = networkErr instanceof Error ? networkErr.message : String(networkErr);
                logger_1.logger.warn('yandexgpt: tool-call network failure, trying next model', {
                    model,
                    reason: msg.slice(0, 160),
                });
                lastError = msg;
            ...
            logger_1.logger.warn('yandexgpt: tool-call model fallback', {
                model,
                status,
                reason: isRateLimit ? 'rate-limit' : 'server-error',
            });
...
class AIRouter {
    constructor() {
        ...
        for (const manifest of (0, provider_manifests_1.loadConfiguredAIProviderManifests)()) {
            if (this.providers.has(manifest.name)) {
                logger_1.logger.warn("Skipping duplicate configured AI provider", {
                    provider: manifest.name,
                });
                continue;
            }
            this.providers.set(manifest.name, (0, provider_manifests_1.createConfiguredAIProvider)(manifest));
    ...
    async chat(messages, options = {}) {
        ...
        if (options.workspaceId) {
            const withinBudget = await (0, cost_tracker_1.checkCostBudget)(options.workspaceId);
            if (!withinBudget) {
        ...
        for (const providerName of fallbackChain) {
            ...
            if (!provider)
                continue;
            const cb = (0, circuit_breaker_1.getCircuitBreaker)(`ai:${providerName}`);
            const recordCost = (0, cost_tracker_1.buildCostRecorder)(providerName, options.model || provider.models[0], messages, { agentId: options.agentId, runId: options.runId, workspaceId: options.workspaceId });
            try {
                ...
                if (providerName !== requested) {
                    logger_1.logger.info("ai-router: fallback provider used", { requested, used: providerName });
                }
                return result;
            }
            catch (err) {
                lastError = err instanceof Error ? err : new Error(String(err));
                if (err instanceof circuit_breaker_1.CircuitOpenError) {
                    logger_1.logger.warn("ai-router: circuit open, trying next provider", { provider: providerName });
                    continue;
                }
                if (isTransientProviderError(lastError)) {
                    logger_1.logger.warn("ai-router: provider error, trying fallback", {
    ...
    async chatWithTools(messages, options) {
        ...
        for (const providerName of fallbackChain) {
            ...
            const recordCost = (0, cost_tracker_1.buildCostRecorder)(providerName, options.model || provider.models[0], messages, { agentId: options.agentId, runId: options.runId, workspaceId: options.workspaceId });
            try {
                const result = await cb.execute(() => provider.chatWithTools(messages, {
                    model: options.model,
                ...
                if (providerName !== requested) {
                    logger_1.logger.info("ai-router: fallback tool-capable provider used", {
                        requested,
                        used: providerName,
                    });
                }
                ...
                lastError = err instanceof Error ? err : new Error(String(err));
                if (err instanceof circuit_breaker_1.CircuitOpenError) {
                    logger_1.logger.warn("ai-router: tool-call circuit open, trying next", { provider: providerName });
                    continue;
                }
                if (isTransientProviderError(lastError)) {
                    logger_1.logger.warn("ai-router: tool-call provider error, trying fallback", {
                        provider: providerName,
                        error: lastError.message.slice(0, 160),
                    });
                    continue;
        ...
        logger_1.logger.warn("ai-router: no tool-capable provider succeeded, degrading to text chat", {
 ...
 */
async function hasAvailableProvider() {
    try {
        ...
        logger_1.logger.error("Error checking providers", { error: error instanceof Error ? error.message : String(error) });
        return false;
    }
}
/**
...
function getProviderName() {
    try {
        ...
        logger_1.logger.error("Error getting provider name", { error: error instanceof Error ? error.message : String(error) });
        return null;
    }
}
// Export singleton instance
exports.aiRouter = getRouter();
