...
exports.getProposalSafetyProfile = getProposalSafetyProfile;
exports.buildApplySafetySummary = buildApplySafetySummary;
function getProposalSafetyProfile(proposal) {
    switch (proposal.type) {
        case "create_tasks":
            return {
                ...
                compensationSummary: "If the draft is wrong, issue a follow-up task patch or discard the draft before it reaches the live board.",
                compensationSteps: [
        ...
        case "update_tasks":
            return {
                ...
                compensationMode: "follow_up_patch",
                compensationSummary: "If the patch is wrong, issue an explicit corrective patch that restores owner, priority, or due-date intent.",
                compensationSteps: [
        ...
        case "reschedule_tasks":
            return {
                ...
                compensationMode: "follow_up_patch",
                compensationSummary: "If the reschedule is wrong, send a compensating patch that restores the prior date or proposes a corrected recovery date.",
                compensationSteps: [
        ...
        case "raise_risks":
            return {
                ...
                compensationSummary: "If a risk is overstated or duplicate, close it explicitly or correct its severity and owner rather than hiding it.",
                compensationSteps: [
        ...
        case "draft_status_report":
            return {
                ...
                compensationSummary: "If the narrative is wrong, replace the draft with a corrected version before delivery; no rollback engine is needed.",
                compensationSteps: [
        ...
        case "notify_team":
            return {
                ...
                compensationSummary: "If the message is wrong, send a correction notice and document the operator decision that superseded the draft.",
                compensationSteps: [
...
function buildApplySafetySummary(proposal) {
    const profile = getProposalSafetyProfile(proposal);
    return {
        ...profile,
        operatorDecision: "manual_apply",
        postApplyState: profile.executionMode === "preview_only" ? "draft_only" : "guarded_execution",
    };
}
