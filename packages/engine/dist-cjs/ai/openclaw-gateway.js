...
exports.buildGatewayPrompt = buildGatewayPrompt;
exports.parseGatewayResult = parseGatewayResult;
exports.invokeOpenClawGateway = invokeOpenClawGateway;
const date_fns_1 = require("date-fns");
const grounding_1 = require("./grounding");
const chat_response_1 = require("./chat-response");
...
function normalizeDueDate(value, index) {
    ...
    return (0, date_fns_1.format)((0, date_fns_1.addDays)(new Date(), index + 2), "yyyy-MM-dd");
}
...
async function consumeSseText(response) {
    ...
    const dispatch = () => {
        ...
        const payloadType = typeof payload?.type === "string" ? payload.type.toLowerCase() : eventType.toLowerCase();
        const payloadStatus = typeof payload?.status === "string" ? payload.status.toLowerCase() : null;
        if (trimmed === "[DONE]") {
            terminal = true;
        }
...
function buildGatewayPrompt(input, runId) {
...
function buildProposal(value, runId) {
    ...
    const inferredType = normalizeActionType(value.type) ??
...
function parseGatewayResult(rawText, runId) {
    ...
    if (!parsed) {
        ...
        const fallbackLine = fallbackSummary
            .split("\n")
            .map((line) => line.trim())
    ...
    if (typeof result.title !== "string" ||
        ...
        return {
            ...
            nextSteps: Array.isArray(result.nextSteps)
                ...
                : [],
            facts: (0, chat_response_1.normalizeChatFacts)(result.facts),
            confidence: (0, chat_response_1.normalizeChatConfidence)(result.confidence),
...
async function invokeOpenClawGateway(input, runId, options) {
    ...
    return (0, grounding_1.attachRunGrounding)(parseGatewayResult(outputText, runId), input);
}
