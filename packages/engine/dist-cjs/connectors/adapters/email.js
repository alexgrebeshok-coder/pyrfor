...
function createEmailConnector(env = process.env, transportFactory) {
    return {
        ...descriptor,
        async getStatus() {
            const checkedAt = new Date().toISOString();
            const missingSecrets = (0, email_client_1.getEmailConnectorMissingSecrets)(env);
            ...
            try {
                const probeResult = await (0, email_client_1.probeEmailTransport)(config, transportFactory);
                if (!probeResult.ok) {
                    return {
                        ...descriptor,
                        status: "degraded",
