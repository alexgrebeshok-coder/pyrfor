...
exports.buildAlertFeed = buildAlertFeed;
exports.buildPortfolioAlerts = buildPortfolioAlerts;
exports.buildProjectAlerts = buildProjectAlerts;
exports.scoreAlert = scoreAlert;
exports.calculateFreshness = calculateFreshness;
exports.summarizeRecommendations = summarizeRecommendations;
...
function buildAlertFeed(snapshot, options = {}) {
...
function buildPortfolioAlerts(snapshot, options = {}) {
    const referenceDate = toDate(options.referenceDate ?? snapshot.generatedAt);
    const locale = (0, locale_1.resolveBriefLocale)(options.locale);
    ...
    if (atRiskProjects.length >= Math.max(1, Math.ceil(snapshot.projects.length * 0.3))) {
        ...
        alerts.push(scoreAlert({
            ...
            summary: locale === "ru"
                ? `${atRiskProjects.length} из ${snapshot.projects.length} ${(0, locale_1.formatProjectNoun)(snapshot.projects.length, locale)} требуют эскалации; лидируют ${(0, locale_1.formatList)(projectNames, locale)}.`
                : `${atRiskProjects.length} of ${snapshot.projects.length} projects currently require escalation, led by ${(0, locale_1.formatList)(projectNames, locale)}.`,
            ...
            detectedAt: latestTimestamp(atRiskProjects.map((project) => project.history.at(-1)?.date ?? snapshot.generatedAt)) ?? snapshot.generatedAt,
            metrics: {
    ...
    if (overdueTasks.length >= 2) {
        ...
        alerts.push(scoreAlert({
            ...
            summary: locale === "ru"
                ? `${overdueTasks.length} ${(0, locale_1.formatTaskNoun)(overdueTasks.length, locale, "overdue")} распределены по ${affectedProjects} ${(0, locale_1.formatProjectNoun)(affectedProjects, locale)}.`
            ...
            detectedAt: latestTimestamp(overdueTasks.map((task) => task.dueDate).filter(isNonEmptyString)) ??
    ...
    if (totalBudgetVarianceRatio >= 0.08) {
        alerts.push(scoreAlert({
            ...
            summary: locale === "ru"
                ? `Портфель идёт с отклонением ${(0, locale_1.formatSignedPercent)(totalBudgetVarianceRatio, locale)} к плану на текущую дату.`
                : `Portfolio spend is ${(0, locale_1.formatSignedPercent)(totalBudgetVarianceRatio, locale)} versus planned-to-date.`,
            whyItMatters: locale === "ru"
...
function buildProjectAlerts(snapshot, projectId, options = {}) {
    const referenceDate = toDate(options.referenceDate ?? snapshot.generatedAt);
    const locale = (0, locale_1.resolveBriefLocale)(options.locale);
    ...
    if (project.progress < 100 &&
        ...
        alerts.push(scoreAlert({
            ...
            detectedAt: (context.overdueTasks.length > 0
                ? latestTimestamp(context.overdueTasks.map((task) => task.dueDate))
                : context.daysToDeadline < 0
                    ? project.dates.end
                    : latestTimestamp(context.blockedTasks.map((task) => task.dueDate))) ??
                snapshot.generatedAt,
            metrics: {
    ...
    if (context.budgetVarianceRatio >= 0.1) {
        alerts.push(scoreAlert({
            ...
            summary: locale === "ru"
                ? `${project.name} идёт с отклонением ${(0, locale_1.formatSignedPercent)(context.budgetVarianceRatio, locale)} к плану на текущую дату (${(0, locale_1.formatCurrency)(context.budgetVariance, project.budget.currency, locale)}).`
                : `${project.name} is ${(0, locale_1.formatSignedPercent)(context.budgetVarianceRatio, locale)} versus planned-to-date (${(0, locale_1.formatCurrency)(context.budgetVariance, project.budget.currency, locale)}).`,
            whyItMatters: locale === "ru"
    ...
    if (context.openRisks.length > 0 &&
        ...
        alerts.push(scoreAlert({
            ...
            detectedAt: latestTimestamp(context.openRisks.map((risk) => risk.updatedAt)) ?? snapshot.generatedAt,
            metrics: {
    ...
    if (context.overloadedTeamMembers.length > 0 ||
        ...
        alerts.push(scoreAlert({
            ...
            detectedAt: latestTimestamp(context.tasks.map((task) => task.dueDate).filter(isNonEmptyString)) ?? snapshot.generatedAt,
            metrics: {
    ...
    if (context.planFact.evidence.pendingWorkReports >= 2 ||
        ...
        alerts.push(scoreAlert({
            ...
            detectedAt: context.planFact.evidence.lastApprovedWorkReportDate ?? snapshot.generatedAt,
            metrics: {
                pendingWorkReports: context.planFact.evidence.pendingWorkReports,
                daysSinceLastApprovedReport: context.planFact.evidence.daysSinceLastApprovedReport ?? 0,
            },
        }, referenceDate));
    }
    if (!alerts.length &&
...
function scoreAlert(draft, referenceDate = new Date()) {
...
function calculateFreshness(detectedAt, referenceDate = new Date()) {
...
function summarizeRecommendations(alerts, locale = "ru", limit = 3) {
...
function buildProjectSignalContext(snapshot, project, referenceDate) {
    ...
    const planFact = (0, service_1.buildProjectPlanFactSummary)(snapshot, project.id, {
        referenceDate,
    });
    return {
        project,
...
function buildScheduleSummary(context, locale) {
    if (context.daysToDeadline < 0) {
        return locale === "ru"
            ? `${context.project.name} уже вышел за плановую дату завершения; открыты ${context.overdueTasks.length} ${(0, locale_1.formatTaskNoun)(context.overdueTasks.length, locale, "overdue")}.`
    ...
    if (context.overdueTasks.length > 0) {
        return locale === "ru"
            ? `${context.overdueTasks.length} ${(0, locale_1.formatTaskNoun)(context.overdueTasks.length, locale, "overdue")} и ${context.blockedTasks.length} ${(0, locale_1.formatTaskNoun)(context.blockedTasks.length, locale, "blocked")} замедляют исполнение в ${context.project.name}.`
    ...
    return locale === "ru"
        ? `${context.blockedTasks.length} ${(0, locale_1.formatTaskNoun)(context.blockedTasks.length, locale, "blocked")} ограничивают исполнение в ${context.project.name}.`
...
function buildResourceSummary(context, locale) {
    if (context.overloadedTeamMembers.length > 0) {
        return locale === "ru"
            ? `${context.overloadedTeamMembers.length} исполнителей уже загружены более чем на 90%, при этом открыты ${context.openTasks.length} ${(0, locale_1.formatTaskNoun)(context.openTasks.length, locale, "open")}.`
    ...
    return locale === "ru"
        ? `${context.openTasks.length} ${(0, locale_1.formatTaskNoun)(context.openTasks.length, locale, "open")} конкурируют за команду из ${context.teamMembers.length} человек.`
