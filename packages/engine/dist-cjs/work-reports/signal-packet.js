...
exports.createWorkReportSignalPacket = createWorkReportSignalPacket;
exports.buildWorkReportSignalRunBlueprints = buildWorkReportSignalRunBlueprints;
const agents_1 = require("../ai/agents");
const server_context_1 = require("../ai/server-context");
...
async function createWorkReportSignalPacket(reportId, request = {}, deps = {}) {
    const now = deps.now ?? (() => new Date());
    const packetIdFactory = deps.packetIdFactory ?? createPacketId;
    const createRun = deps.createRun ?? server_runs_1.createServerAIRun;
    const loadWorkReport = deps.loadWorkReport ?? service_2.getWorkReportById;
    const loadSnapshot = deps.loadSnapshot ?? ((input) => (0, snapshot_1.loadExecutiveSnapshot)(input));
    const loadContext = deps.loadContext ??
        ((input) => (0, server_context_1.loadServerAIContext)({
    ...
    const planFact = (0, service_1.buildProjectPlanFactSummary)(snapshot, report.projectId);
    ...
    const runs = await Promise.all(blueprints.map(async (blueprint) => ({
...
function buildWorkReportSignalRunBlueprints(context, report, signal, locale = context.locale, traceMeta) {
...
function buildSignalSnapshot(report, snapshot, planFact, locale) {
    const topAlerts = (0, scoring_1.buildProjectAlerts)(snapshot, report.projectId, {
        locale: (0, locale_1.resolveBriefLocale)(locale),
    }).slice(0, 3);
...
function requireAgent(agentId) {
    const agent = (0, agents_1.getAgentById)(agentId);
    if (!agent) {
