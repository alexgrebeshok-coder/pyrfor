...
exports.getEvidenceFusionOverview = getEvidenceFusionOverview;
exports.buildEvidenceFusionFacts = buildEvidenceFusionFacts;
...
async function getEvidenceFusionOverview(query = {}, deps = {}) {
    const now = deps.now ?? (() => new Date());
    const getEvidence = deps.getEvidence ?? service_1.getEvidenceLedgerOverview;
...
function buildEvidenceFusionFacts(records) {
...
function buildFusionFact(report, videoFacts, gpsSessions) {
    ...
    const reportNumber = readMetadataString(reportMetadata, "reportNumber") ?? report.sourceRef ?? report.entityRef;
...
function buildExplanation(report, supports, verificationStatus, confidence) {
    const reportNumber = readMetadataString(report.metadata, "reportNumber") ?? report.sourceRef ?? report.entityRef;
