...
exports.contextBuilder = exports.memoryManager = void 0;
...
function getMemoryStorage() {
    ...
    try {
        ...
        logger_1.logger.error("Memory read error", { error: error instanceof Error ? error.message : String(error) });
        return [];
    }
}
function setMemoryStorage(entries) {
    ...
    try {
        ...
        logger_1.logger.error("[Memory] Error writing:", { error: error instanceof Error ? error.message : String(error) });
    }
}
// ============================================
// Memory Manager
...
exports.memoryManager = {
    ...
    update(id, updates) {
        ...
        setMemoryStorage(entries);
        logger_1.logger.info("Updated", { key: entries[index].key });
    ...
    cleanup() {
        ...
        if (removed > 0) {
            ...
            logger_1.logger.warn("Cleaned up invalid entries", { count: removed });
        }
        return removed;
    },
    /**
    ...
    import(jsonString) {
        try {
            ...
            logger_1.logger.info("Memory imported", { count: entries.length });
            return entries.length;
        }
        catch (error) {
            logger_1.logger.error("Import error", { error: error instanceof Error ? error.message : String(error) });
            return 0;
        }
    },
};
...
exports.contextBuilder = {
     ...
     */
    build(options) {
        const memories = exports.memoryManager.getAll({
    ...
    buildProjectContext(projectId) {
        const memories = exports.memoryManager.getAll().filter((e) => e.tags.includes(projectId));
...
function initializeDefaultMemories() {
    const existing = exports.memoryManager.getAll();
    if (existing.length > 0) {
        logger_1.logger.info("Memory already initialized");
        return;
    }
    // Add default memories
    exports.memoryManager.add({
        type: "procedural",
        category: "skill",
        key: "ai-provider",
        value: "openrouter",
