...
exports.levelResources = levelResources;
const auto_schedule_1 = require("./auto-schedule");
const critical_path_1 = require("./critical-path");
function levelResources(input) {
    const scheduled = (0, auto_schedule_1.autoScheduleTasks)(input);
    ...
    for (const assignment of input.assignments) {
        ...
        const startOffset = (0, critical_path_1.getDayOffset)(metric.earliestStart, scheduled.criticalPath.projectStart);
    ...
    for (const [resourceKey, resourceTimeline] of loadMap) {
        const capacity = capacityMap.get(resourceKey) ??
            {
                resourceKey,
        ...
        for (const [offset, bucket] of resourceTimeline) {
            ...
            conflicts.push({
                ...
                date: (0, critical_path_1.addDays)(scheduled.criticalPath.projectStart, offset),
    ...
    const adjustments = [...shiftByTask.entries()].map(([taskId, shiftDays]) => {
        ...
        return {
            ...
            title: metric.title,
            shiftDays,
            newStartDate: (0, critical_path_1.addDays)(metric.earliestStart, shiftDays),
            newDueDate: (0, critical_path_1.addDays)(metric.earliestFinish, shiftDays),
