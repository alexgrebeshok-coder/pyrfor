...
exports.createSchedulingId = createSchedulingId;
exports.getProjectSchedulingContext = getProjectSchedulingContext;
exports.serializeCriticalPath = serializeCriticalPath;
const prisma_1 = require("../prisma");
function createSchedulingId(prefix) {
    return `${prefix}-${Date.now().toString(36)}-${Math.random().toString(36).slice(2, 6)}`;
}
async function getProjectSchedulingContext(projectId) {
    ...
    const [tasks, assignments] = await Promise.all([
        ...
        prisma_1.prisma.resourceAssignment.findMany({
            where: {
                task: {
                    projectId,
                },
...
function serializeCriticalPath(result) {
    return {
