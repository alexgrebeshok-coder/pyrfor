...
exports.rateLimiter = exports.AgentRateLimiter = void 0;
exports.configureRateLimiterStore = configureRateLimiterStore;
class AgentRateLimiter {
     ...
     */
    async canRequestAsync(provider) {
        const limit = this.limits.get(provider);
        if (!limit) {
        ...
        if (this.store) {
            try {
                ...
                const { count, ttlMs } = await this.store.incrementWindow(key, limit.windowMs);
     ...
     */
    async getWaitTimeAsync(provider) {
        if (!this.store)
...
exports.rateLimiter = new AgentRateLimiter();
/**
 ...
 */
function configureRateLimiterStore(store) {
    exports.rateLimiter.setStore(store);
}
