...
exports.getWorkflowRunDetail = getWorkflowRunDetail;
exports.syncWorkflowStepFromHeartbeatRun = syncWorkflowStepFromHeartbeatRun;
...
async function ensureWorkflowAgentsExist(workspaceId, definition) {
    ...
    const existingAgents = await prisma_1.prisma.agent.findMany({
        where: {
            workspaceId,
            id: { in: agentIds },
        },
...
function computeOutputNodeIds(definition) {
    ...
    const dependencyIds = new Set(definition.nodes.flatMap((node) => node.dependsOn ?? []));
    return definition.nodes
        .map((node) => node.id)
        .filter((nodeId) => !dependencyIds.has(nodeId));
}
async function loadWorkflowRunInternal(workflowRunId) {
    const run = await prisma_1.prisma.workflowRun.findUnique({
...
async function hydrateWorkflowRun(workflowRunId) {
    ...
    const [approvals, delegations] = await Promise.all([
        approvalIds.length > 0
            ...
            : Promise.resolve([]),
        (0, delegation_service_1.listWorkflowDelegations)(workflowRunId),
    ]);
...
async function queueAgentStep(run, step, node, stepsByNodeId) {
    const claimed = await prisma_1.prisma.workflowRunStep.updateMany({
    ...
    const childRun = await prisma_1.prisma.heartbeatRun.create({
        data: {
    ...
    try {
        await job_queue_1.jobQueue.enqueue({
            agentId: node.agentId,
            reason: "event",
        ...
        if (parentAgentStep?.agentId && parentAgentStep.heartbeatRunId) {
            await (0, delegation_service_1.createAgentDelegation)({
                workspaceId: run.workspaceId,
                workflowRunId: run.id,
                workflowStepId: step.id,
        ...
        (0, sse_1.broadcastSSE)("workflow_step_queued", {
            workflowRunId: run.id,
            workflowStepId: step.id,
            heartbeatRunId: childRun.id,
            agentId: node.agentId,
...
async function createApprovalStep(run, step, node) {
    const claimed = await prisma_1.prisma.workflowRunStep.updateMany({
        where: { id: step.id, status: "pending" },
        data: {
    ...
    const approval = await prisma_1.prisma.approval.create({
        data: {
            id: `apr-wf-${(0, crypto_1.randomUUID)()}`,
    ...
    (0, sse_1.broadcastSSE)("workflow_step_waiting_approval", {
        workflowRunId: run.id,
        workflowStepId: step.id,
        approvalId: approval.id,
    });
}
async function syncExistingStepState(run) {
    ...
    const [heartbeatRuns, approvals] = await Promise.all([
        stepRunIds.length > 0
            ? prisma_1.prisma.heartbeatRun.findMany({
            ...
            if (nextStatus === "failed" && step.attemptCount < step.maxRetries) {
                await prisma_1.prisma.workflowRunStep.update({
                    ...
                    data: {
                        ...
                        errorMessage: parseObject(heartbeatRun.resultJson).error?.toString() ??
...
async function updateWorkflowRunStatus(workflowRunId, definition) {
    ...
    if (status === "succeeded" || status === "failed" || status === "cancelled") {
        (0, sse_1.broadcastSSE)("workflow_run_completed", {
            workflowRunId,
            status,
        });
    }
}
async function listWorkflowTemplates(workspaceId, status) {
    const templates = await prisma_1.prisma.workflowTemplate.findMany({
        where: {
            workspaceId,
            ...(status ? { status } : {}),
        },
    ...
    return templates.map((template) => ({
        ...
        recentRunStats: summarizeSteps(template.runs.map((run) => ({
...
async function createWorkflowTemplate(input) {
    const definition = parseWorkflowDefinition(input.definition);
    await ensureWorkflowAgentsExist(input.workspaceId, definition);
    const template = await prisma_1.prisma.workflowTemplate.create({
        data: {
            workspaceId: input.workspaceId,
            name: input.name,
            slug: (input.slug?.trim() || (0, utils_1.slugify)(input.name)).slice(0, 60),
    ...
    (0, sse_1.broadcastSSE)("workflow_template_created", {
...
async function updateWorkflowTemplate(templateId, input) {
    const existing = await prisma_1.prisma.workflowTemplate.findUnique({
        where: { id: templateId },
    });
    if (!existing) {
    ...
    const updated = await prisma_1.prisma.workflowTemplate.update({
        where: { id: templateId },
        data: {
...
async function createWorkflowRun(input) {
    const template = await prisma_1.prisma.workflowTemplate.findFirst({
        where: {
            id: input.templateId,
            workspaceId: input.workspaceId,
        },
    ...
    const runInput = typeof input.input === "string"
        ? { prompt: input.input }
        : input.input ?? {};
    const workflowRun = await prisma_1.prisma.$transaction(async (tx) => {
    ...
    (0, sse_1.broadcastSSE)("workflow_run_created", {
        workflowRunId: workflowRun.id,
        templateId: template.id,
    });
    return advanceWorkflowRun(workflowRun.id);
}
async function advanceWorkflowRun(workflowRunId) {
    ...
    const readySteps = refreshedRun.steps.filter((step) => {
        ...
        return dependencies.every((dependencyId) => {
            ...
            return dependencyStep?.status === "succeeded";
        });
    });
...
async function listWorkflowRuns(workspaceId, options = {}) {
...
async function getWorkflowRunDetail(workflowRunId) {
    return hydrateWorkflowRun(workflowRunId);
}
async function syncWorkflowStepFromHeartbeatRun(runId) {
    const heartbeatRun = await prisma_1.prisma.heartbeatRun.findUnique({
        where: { id: runId },
        include: {
            checkpoints: {
                orderBy: { createdAt: "desc" },
    ...
    const step = await prisma_1.prisma.workflowRunStep.findUnique({
    ...
    await (0, delegation_service_1.updateDelegationStatusByChildRun)(heartbeatRun.id, mapHeartbeatRunStatusToDelegationStatus(heartbeatRun.status), {
        workflowRunId,
        workflowStepId,
    });
