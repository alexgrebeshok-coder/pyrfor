...
function parseFrontmatter(fm: string): Record<string, unknown> {
  ...
  const currentList: Record<string, string>[] = [];
 ...
 */
export function parseSkillMd(raw: string, sourcePath?: string): ParsedSkill | null {
  const parts = splitFrontmatter(raw);
  if (!parts) {
    console.warn(
      `[skill-md-parser] Missing frontmatter in skill file${sourcePath ? ` (${sourcePath})` : ''} — skipping.`,
    );
    return null;
  }
  ...
  if (!fm['name'] || typeof fm['name'] !== 'string' || !fm['name'].trim()) {
    console.warn(
      `[skill-md-parser] Frontmatter missing required field "name"${sourcePath ? ` in ${sourcePath}` : ''} — skipping.`,
    );
    return null;
  }
  ...
  if (typeof fm['trigger'] === 'string') parsed.trigger = fm['trigger'].trim();
  if (typeof fm['icon'] === 'string') parsed.icon = fm['icon'].trim();
  if (typeof fm['category'] === 'string') parsed.category = fm['category'].trim();
  if (Array.isArray(fm['parameters'])) {
    parsed.parameters = fm['parameters'] as Array<{ name: string; type?: string }>;
  }
  if (sourcePath) parsed.sourcePath = sourcePath;
