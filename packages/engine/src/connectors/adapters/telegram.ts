...
const descriptor: ConnectorDescriptor = {
  ...
  name: "Telegram",
  description:
    "Bridge for bot commands and outbound PMO notifications. Uses the existing webhook endpoint and now performs a live Bot API readiness probe.",
    ...
    async getStatus(): Promise<ConnectorStatus> {
      ...
      try {
        ...
        const [botProfileResult, webhookInfoResult] = await Promise.all([
          probeTelegramMethod<TelegramBotProfile>(token, "getMe", probeFetch),
          probeTelegramMethod<TelegramWebhookInfo>(token, "getWebhookInfo", probeFetch),
        ]);
        ...
        return {
          ...
          metadata: {
            ...
            hasCustomCertificate:
              webhookInfoResult.result.has_custom_certificate ?? null,
          },
        };
      } catch (error) {
        return {
