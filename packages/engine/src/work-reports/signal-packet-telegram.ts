...
} from '../connectors/telegram-client';
...
function clampTelegramMessage(value: string) {
...
function buildWorkReportSignalPacketTelegramDigest(packet: WorkReportSignalPacketPortable) {
  const lines = [
  ...
  if (alerts.length === 0) {
    ...
    alerts.forEach((alert, index) => {
      lines.push(
        `${index + 1}. [${alert.severity.toUpperCase()}] ${truncateText(alert.title, 90)} — ${truncateText(alert.summary, 160)}`
      );
    });
  }
  ...
  runs.forEach((entry) => {
    lines.push(
      `- ${entry.label} (${entry.run.status}): ${truncateText(entry.run.result?.summary ?? "AI run ещё не вернул summary.", 160)}`
    );
    if (entry.run.result?.proposal) {
      lines.push(
        `  Proposal: ${truncateText(entry.run.result.proposal.title, 80)} [${entry.run.result.proposal.state}]`
      );
    }
  });
...
export async function deliverWorkReportSignalPacketToTelegram(
  request: WorkReportSignalPacketTelegramDeliveryRequest,
  deps: WorkReportSignalPacketTelegramDeliveryDeps = {}
): Promise<WorkReportSignalPacketTelegramDeliveryResult> {
  ...
  const chatId = trimOptionalString(request.chatId) ?? getTelegramDefaultChatId(env);
