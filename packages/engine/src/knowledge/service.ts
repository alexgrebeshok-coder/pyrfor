...
export async function getKnowledgeLoopOverview(
  query: KnowledgeLoopQuery = {},
  deps: KnowledgeServiceDeps = {}
): Promise<KnowledgeLoopOverview> {
  const now = deps.now ?? (() => new Date());
  const getEscalations = deps.getEscalations ?? getEscalationQueueOverview;
  const escalations =
    deps.escalations ??
...
function buildPatternGroups(items: EscalationRecordView[]): PatternGroup[] {
...
function buildPlaybook(group: PatternGroup): KnowledgePlaybookView {
...
function buildActiveGuidance(
  ...
  return items
    ...
    .filter((item): item is KnowledgeGuidanceView => item !== null)
    .sort((left, right) => urgencyPriority(left.urgency) - urgencyPriority(right.urgency));
}

function buildBenchmark(items: EscalationRecordView[]): KnowledgeBenchmarkView {
  ...
  const resolutionRate = ratio(
    items.filter((item) => item.queueStatus === "resolved").length,
    items.length
  );
  const breachRate = ratio(
    items.filter((item) => item.slaState === "breached").length,
    items.length
  );
...
function buildPlaybookTitle(proposalType: AIActionType | null, purpose: string | null) {
  if (proposalType === "create_tasks") return "Task drafting playbook";
  if (proposalType === "update_tasks") return "Execution patch playbook";
  if (proposalType === "reschedule_tasks") return "Schedule recovery playbook";
  if (proposalType === "raise_risks") return "Risk surfacing playbook";
  if (proposalType === "draft_status_report") return "Executive narrative playbook";
  if (proposalType === "notify_team") return "Team communication playbook";
  if (purpose === "tasks") return "Task intervention playbook";
  if (purpose === "risks") return "Risk intervention playbook";
  if (purpose === "status") return "Status intervention playbook";
...
function buildLessons(
  ...
  if (items.some((item) => !item.owner)) {
    lessons.add(
      `First move is explicit owner assignment${benchmark.ownerRole ? ` to ${benchmark.ownerRole}` : ""}.`
    );
  }
...
function buildBenchmarkSummary(benchmark: KnowledgeBenchmarkView) {
  const ownerLabel = benchmark.ownerRole ? `${benchmark.ownerRole} owner` : "named owner";
  return `${ownerLabel} · acknowledge within ${benchmark.ackTargetHours}h · resolution rate ${Math.round(
    benchmark.resolutionRate * 100
  )}%`;
}

function buildGuidanceAction(
  item: EscalationRecordView,
  playbook: KnowledgePlaybookView
) {
...
function getProposalSafetyProfileByType(proposalType: AIActionType) {
  switch (proposalType) {
    case "create_tasks":
...
function createSafetyTemplateProposal(type: AIActionType): AIActionProposal {
  const base = {
...
function isProposalType(value: string | undefined): value is AIActionType {
  return (
    value === "create_tasks" ||
    value === "update_tasks" ||
...
function deriveEscalationLimit(limit: number | undefined) {
...
function mostCommonRole(items: EscalationRecordView[]) {
...
function urgencyPriority(value: string) {
  switch (value) {
    case "critical":
      return 0;
    case "high":
