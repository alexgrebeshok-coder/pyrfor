...
export type FindingKind =
  | 'hardcoded_secret'       // api keys, tokens, private keys
  | 'sql_injection_risk'     // string-concat into SQL
  | 'command_injection_risk' // child_process exec with template-string
  | 'path_traversal_risk'    // unchecked path join from input
  | 'insecure_random'        // Math.random for secrets/tokens
  | 'weak_crypto'            // md5, sha1 for security purposes
  | 'eval_use'               // eval / new Function
  | 'unsafe_yaml_load'       // yaml.load without schema
  | 'cors_wildcard'          // Access-Control-Allow-Origin: *
  | 'http_url_in_prod'       // http:// (not https) literal
  | 'todo_security_marker';  // TODO/FIXME mentioning auth/security
...
const ALL_KINDS: readonly FindingKind[] = [
  'hardcoded_secret',
  'sql_injection_risk',
...
const REMEDIATION: Record<FindingKind, string> = {
  ...
  sql_injection_risk:
    'Use parameterised queries or prepared statements instead of ' +
    'string concatenation or unescaped template literals.',
  command_injection_risk:
    'Pass commands as argument arrays to spawn(); validate and ' +
    'sanitise all user-supplied input before any shell execution.',
  path_traversal_risk:
    'Validate and normalise paths; resolve against a fixed base ' +
    'directory and reject paths that escape it.',
  insecure_random:
    'Use crypto.randomBytes() or crypto.getRandomValues() for ' +
    'security-sensitive random values.',
  weak_crypto:
    'Replace MD5/SHA-1 with SHA-256 or stronger (SHA-3, BLAKE2) ' +
    'for all security-critical hashing.',
  eval_use:
    'Avoid eval() and new Function(); use JSON.parse() or a ' +
    'purpose-built sandboxed evaluator instead.',
  unsafe_yaml_load:
    'Pass an explicit safe schema: yaml.load(str, { schema: yaml.FAILSAFE_SCHEMA }) ' +
    'or switch to yaml.safeLoad() to prevent arbitrary code execution.',
  cors_wildcard:
    'Restrict Access-Control-Allow-Origin to a known allowlist of origins; ' +
    'never use a wildcard on authenticated APIs.',
  http_url_in_prod:
    'Switch to HTTPS endpoints in production to prevent eavesdropping ' +
    'and traffic interception.',
  todo_security_marker:
    'Resolve the outstanding security/auth TODO before shipping to production; ' +
    'track it in your issue tracker.',
};

// ====== Message Generators ======

function makeMessage(kind: FindingKind, line: number): string {
  switch (kind) {
...
interface PatternDef {
  kind: FindingKind;
  /** Primary regex tested against the source line. */
  regex: RegExp;
  severity: Severity;
  confidence: number;
  /**
   * When set, the regex match is only counted as a finding if this secondary
   * regex also matches the same line (used for context-dependent heuristics).
   */
  contextRequired?: RegExp;
  /**
   * When set, the regex match is suppressed if this regex matches the
   * matched text (used e.g. to filter localhost URLs).
   */
  excludeMatch?: RegExp;
}
 ...
 */
export function scanFile(file: SourceFileInput): Finding[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    for (const pat of PATTERNS) {
      ...
      if (pat.contextRequired && !pat.contextRequired.test(rawLine)) continue;

      // Exclusion guard: matched text must not match the exclusion pattern.
      if (pat.excludeMatch && pat.excludeMatch.test(match[0])) continue;
...
function zeroSeverityMap(): Record<Severity, number> {
...
function zeroKindMap(): Record<FindingKind, number> {
  return Object.fromEntries(
    ALL_KINDS.map(k => [k, 0]),
...
export async function runSecurityReviewer(
  input: SecurityReviewerInput,
): Promise<SecurityReviewerOutput> {
  const {
    files,
    ignorePatterns = [],
    maxFindings,
...
export function subagentSpec(): {
  ...
  return {
    name: 'security-reviewer',
    description:
      'Scans a set of caller-supplied source-file blobs using regex-based ' +
