...
export interface FixProposal {
  ...
  suggestedActions: Array<
    | { kind: 'update_assertion'; targetFile: string; hint: string }
    | { kind: 'fix_source';       targetFile: string; hint: string }
    | { kind: 'increase_timeout'; targetFile: string; newMs: number }
    | { kind: 'regenerate_snapshot'; targetFile: string }
    | { kind: 'install_dep';      pkg: string }
    | { kind: 'investigate';      hint: string }
  >;
 ...
 */
export function parseTimeoutMs(message: string): number | null {
 ...
 */
export function classifyFailure(t: FailingTest): FailureClassification {
  const haystack = `${t.errorMessage}\n${t.errorStack ?? ''}`;

  const matched: Array<{ category: FailureCategory; signal: string }> = [];
  ...
  switch (c.category) {
    case 'assertion_mismatch':
      return {
        ...
        category: c.category,
        rationale:
          'The assertion received a value that did not match the expectation. ' +
    ...
    case 'thrown_error':
      return {
        ...
        category: c.category,
        rationale:
          'An unexpected exception was thrown during the test. This usually means ' +
    ...
    case 'timeout': {
      ...
      const newMs  = parsed !== null ? parsed * 2 : 10_000;
      return {
        testId: t.id,
        category: c.category,
        rationale:
          'The test exceeded its configured time limit. This may indicate an async ' +
    ...
    case 'snapshot_mismatch':
      return {
        ...
        category: c.category,
        rationale:
          'The rendered output no longer matches the stored snapshot. If the change ' +
    ...
    case 'import_error': {
      ...
      return {
        ...
        category: c.category,
        rationale:
          'A module could not be resolved at import time. This is typically caused ' +
    ...
    case 'unhandled_rejection':
      return {
        ...
        category: c.category,
        rationale:
          'A promise was rejected without a handler. This typically means an async ' +
    ...
    default:
      return {
        ...
        category: 'unknown',
        rationale:
          'The failure could not be classified by any known heuristic. Manual ' +
 ...
 */
function zeroCategoryMap(): Record<FailureCategory, number> {
  return Object.fromEntries(ALL_CATEGORIES.map(c => [c, 0])) as Record<FailureCategory, number>;
}

/**
 * Build a zero-filled byRisk summary map.
 */
function zeroRiskMap(): Record<'low' | 'medium' | 'high', number> {
 ...
 */
export async function runTestFixer(input: TestFixerInput): Promise<TestFixerOutput> {
  const { failures, maxProposals } = input;
  const cap = maxProposals !== undefined ? maxProposals : failures.length;

  const classifications: FailureClassification[] = failures.map(classifyFailure);

  const allProposals: FixProposal[] = failures.map((t, i) =>
    proposeFix(t, classifications[i]),
  );
...
export function subagentSpec(): {
  ...
  return {
    name: 'test-fixer',
    description:
      'Classifies failing tests from a test-runner JSON report into well-known ' +
