...
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export function setTelegramPrismaClient(client: any): void {
  _prisma = client;
}

// eslint-disable-next-line @typescript-eslint/no-explicit-any
export function getTelegramPrismaClient(): any {
  if (!_prisma) {
 ...
 */
export function createRateLimiter(perMinute: number): { allow(chatId: number): boolean } {
  ...
  return {
    allow(chatId: number): boolean {
      ...
      let timestamps = windows.get(chatId) ?? [];
...
export async function handleStatus({ chatId }: CommandArgs): Promise<string> {
  ...
  const totalActive = projects.filter((p: { status: string }) => p.status === 'active').length;
  const totalAtRisk = projects.filter((p: { status: string }) => p.status === 'at-risk').length;
  lines.push('');
...
export async function handleProjects({ chatId }: CommandArgs): Promise<string> {
  const prisma = getTelegramPrismaClient();
...
export async function handleTasks({ chatId }: CommandArgs): Promise<string> {
  const prisma = getTelegramPrismaClient();
  ...
  if (tasks.length === 0) return '✅ Все задачи выполнены! Нет открытых задач.';
  ...
  for (const task of tasks) {
    ...
    const projectStr = task.project?.name ? ` [${escapeMarkdown(task.project.name)}]` : '';

    lines.push(`${statusEmoji}${priorityEmoji} *${escapeMarkdown(task.title)}*${projectStr}${dueStr}`);
  }

  const blockedCount = tasks.filter((t: { status: string }) => t.status === 'blocked').length;
  if (blockedCount > 0) lines.push(`\n⚠️ Заблокировано: ${blockedCount}`);

  return lines.join('\n');
}

export async function handleAddTask({ chatId, params }: CommandArgs): Promise<string> {
  const prisma = getTelegramPrismaClient();

  const projectQuery = params[0] ?? '';
  ...
  if (!project) {
    const projects = await prisma.project.findMany({ select: { name: true }, take: 5 });
    const suggestions = projects.map((p: { name: string }) => `• ${p.name}`).join('\n');
...
export async function handleAi(
  { chatId, params }: CommandArgs,
  runMessage?: (text: string) => Promise<string>,
): Promise<string> {
...
export async function handleMorningBrief({ chatId }: CommandArgs): Promise<string> {
  const prisma = getTelegramPrismaClient();
  ...
  const atRisk = projects.filter((p: { status: string }) => p.status === 'at-risk');
