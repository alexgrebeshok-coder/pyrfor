...
export interface PromptExperiment {
  id: string;
  project: string;              // e.g. 'global' | 'freeclaude' | 'codex'
  agent?: string;               // optional agent narrowing
  hypothesis: string;
  status: ExperimentStatus;
  variants: PromptVariant[];
  metrics: Record<string, ExperimentMetrics>;  // keyed by variant id
  minSamplesPerVariant: number;
  successCriterion: 'success_rate' | 'latency' | 'cost' | 'composite';
  significanceDelta: number;    // e.g. 0.05 = 5 pp difference required
  createdAt: string;
  decidedAt?: string;
  winner?: string;              // variant id of declared winner
}
...
export interface CreateExperimentInput {
  ...
  agent?: string;
  hypothesis: string;
  variants: Array<{ label: string; prompt: string; weight?: number }>;
  minSamplesPerVariant?: number;                          // default 10
  successCriterion?: PromptExperiment['successCriterion']; // default 'success_rate'
  significanceDelta?: number;                             // default 0.05
}
...
export interface PromptEngineer {
  ...
  pickVariant(opts: {
    ...
    agent?: string;
    rng?: () => number;
  }): { experimentId: string; variantId: string; prompt: string } | null;
...
export interface PromptEngineerOptions {
  filePath?: string;  // default ~/.pyrfor/prompt-experiments.json
}
...
function emptyMetrics(): ExperimentMetrics {
  return {
    sessions: 0,
    successes: 0,
    failures: 0,
...
export function createPromptEngineer(opts?: PromptEngineerOptions): PromptEngineer {
  const filePath =
    opts?.filePath ?? path.join(homedir(), '.pyrfor', 'prompt-experiments.json');

  const _experiments = new Map<string, PromptExperiment>();

  // ── createExperiment ────────────────────────────────────────────────────────

  function createExperiment(input: CreateExperimentInput): PromptExperiment {
    const id = generateId();
    const now = new Date().toISOString();

    const variants: PromptVariant[] = input.variants.map((v) => ({
      id: generateId(),
      label: v.label,
      prompt: v.prompt,
      weight: v.weight ?? 1,
    ...
    const exp: PromptExperiment = {
      ...
      variants,
      metrics,
      minSamplesPerVariant: input.minSamplesPerVariant ?? 10,
      successCriterion: input.successCriterion ?? 'success_rate',
      significanceDelta: input.significanceDelta ?? 0.05,
      createdAt: now,
    };
  ...
  function start(experimentId: string): PromptExperiment | null {
    const exp = _experiments.get(experimentId);
    if (!exp || exp.status !== 'draft') return null;
    const updated: PromptExperiment = { ...exp, status: 'running' };
  ...
  function archive(experimentId: string): PromptExperiment | null {
    const exp = _experiments.get(experimentId);
    if (!exp) return null;
    const updated: PromptExperiment = { ...exp, status: 'archived' };
    _experiments.set(experimentId, updated);
    return updated;
  }
  ...
  function pickVariant(opts: {
    ...
    const candidates = Array.from(_experiments.values()).filter((exp) => {
      ...
      if (exp.project !== opts.project) return false;
      if (exp.agent !== undefined && exp.agent !== opts.agent) return false;
      return true;
    });
    ...
    const getWeight = nonZero.length > 0 ? (v: PromptVariant) => v.weight : () => 1;
  ...
  function recordOutcome(
    ...
    const vm = { ...(metrics[variantId] ?? emptyMetrics()) };
    vm.sessions++;
    if (outcome.success) vm.successes++;
    else vm.failures++;
    vm.totalLatencyMs += outcome.latencyMs;
    vm.totalTokensIn += outcome.tokensIn ?? 0;
    vm.totalTokensOut += outcome.tokensOut ?? 0;
    vm.totalCostUsd += outcome.costUsd ?? 0;
    metrics[variantId] = vm;

    const updated: PromptExperiment = { ...exp, metrics };
    _experiments.set(experimentId, updated);
    ...
    if (updated.status === 'running') {
      const allReached = updated.variants.every(
        (v) => (metrics[v.id]?.sessions ?? 0) >= updated.minSamplesPerVariant,
      );
      if (allReached) {
  ...
  function evaluate(experimentId: string): {
    decided: boolean;
    status: ExperimentStatus;
    winner?: string;
    reason: string;
    ...
    const allReached = variants.every(
      (v) => (exp.metrics[v.id]?.sessions ?? 0) >= exp.minSamplesPerVariant,
    );
    if (!allReached) {
      return {
        decided: false,
    ...
    const controlVariant = variants.find((v) => v.label === 'control') ?? variants[0];

    // Defaults overwritten by each criterion branch.
    let winner: string | undefined = undefined;
    let newStatus: ExperimentStatus = 'inconclusive';
    let reason = '';
    ...
    switch (exp.successCriterion) {
      ...
      case 'latency': {
        ...
        const relLat =
          worstLat.avg > 0 ? (worstLat.avg - bestLat.avg) / worstLat.avg : 0;
      ...
      case 'cost': {
        ...
        const relCost =
          worstCost.avg > 0 ? (worstCost.avg - bestCost.avg) / worstCost.avg : 0;
      ...
      case 'composite': {
        ...
        const scored = variants.map((v, i) => {
          ...
          const latNorm =
            maxLat > minLat ? (avgLats[i] - minLat) / (maxLat - minLat) : 0;
          const costNorm =
            maxCost > minCost ? (avgCosts[i] - minCost) / (maxCost - minCost) : 0;
    ...
    const finalExp: PromptExperiment = {
      ...exp,
      status: newStatus,
      decidedAt: new Date().toISOString(),
      winner,
  ...
  function get(experimentId: string): PromptExperiment | null {
