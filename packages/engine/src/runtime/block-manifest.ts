...
import { isValidMemoryTableName } from './block-memory-namespace';
...
function validateContractRefs(
  ...
  for (const [index, item] of value.entries()) {
    ...
    if ('optional' in item) requireBoolean(item, 'optional', errors, `${pathPrefix}.${index}`);
    if (options.allowSchema && 'schema' in item) validateContractSchema(item.schema, `${pathPrefix}.${index}.schema`, errors);
  }
}

function validateContractSchema(value: unknown, pathPrefix: string, errors: BlockManifestIssue[]): void {
  ...
  if ('mediaType' in schema) requireString(schema, 'mediaType', errors, pathPrefix);
  if ('sha256' in schema) {
  ...
  if ('validate' in schema) requireBoolean(schema, 'validate', errors, pathPrefix);
...
function validateMemoryScopeCapabilities(
  memoryScopeRaw: unknown,
  capabilitiesRaw: unknown,
  errors: BlockManifestIssue[],
  warnings: BlockManifestIssue[],
): void {
  if (memoryScopeRaw === undefined) return;
  ...
  for (const tier of ['project_shared', 'block_private', 'global_shared'] as const) {
    ...
    if (tables === undefined || !Array.isArray(tables)) continue;
...
function hasMemoryCapability(capabilities: Record<string, unknown>[], scope: string, access?: 'read' | 'write'): boolean {
  return capabilities.some((capability) => {
    if (capability.scope !== scope) return false;
    if (access) return capability.token === `memory:${access}`;
