...
export class SkillsLibrary {
  ...
  search(query: string): Skill[] {
    ...
    for (const skill of this._skills.values()) {
      ...
      if (skill.tags.some((t) => t.toLowerCase().includes(q))) score += 2;
   ...
   */
  findRelevant(taskDescription: string, limit = 5): Skill[] {
    ...
    for (const skill of this._skills.values()) {
      ...
      for (const rule of skill.whenToUse) {
        ...
        for (const word of words) {
          if (ruleLower.includes(word)) score += 2;
        }
        if (desc.includes(ruleLower)) score += 3;
      }

      // fallback: search scoring
      if (skill.name.toLowerCase().includes(desc.slice(0, 30))) score += 3;
      if (skill.tags.some((t) => desc.includes(t.toLowerCase()))) score += 2;

      if (score > 0) scored.set(skill.id, score);
    }
...
export function createSkillsLibrary(initial: Skill[] = []): SkillsLibrary {
...
export const BUILTIN_SKILLS: Skill[] = [
...

  {
    ...
    systemPrompt:
      'You are an expert software engineer specialising in code quality and clean architecture. ' +
...

  {
    ...
    systemPrompt:
      'You are a methodical debugger. You treat debugging as a scientific process: observe symptoms, form hypotheses, design experiments, and eliminate possibilities. ' +
...

  {
    ...
    systemPrompt:
      'You are a senior engineer conducting a constructive code review. ' +
...

  {
    ...
    systemPrompt:
      'You are an expert in test-driven development. You write tests that are readable, deterministic, and fast. ' +
...

  {
    ...
    systemPrompt:
      'You are a TypeScript expert who understands the type system deeply, including conditional types, mapped types, template literal types, and variance. ' +
...

  {
    ...
    systemPrompt:
      'You are a Rust expert with deep knowledge of ownership, borrowing, lifetimes, and the trait system. ' +
...

  {
    ...
    systemPrompt:
      'You are a performance engineer who never optimises without data. ' +
...

  {
    ...
    systemPrompt:
      'You are an observability expert. You add logs that are actionable, structured (JSON fields), and levelled correctly: ERROR for failures requiring attention, WARN for recoverable anomalies, INFO for significant business events, DEBUG for developer diagnostics. ' +
...

  {
    ...
    systemPrompt:
      'You are a technical writer who writes documentation for developers. ' +
...

  {
    ...
    systemPrompt:
      'You are a dependency migration expert. You approach migrations methodically: read the migration guide, catalogue all usages of the old API, apply changes systematically, and verify with tests. ' +
...

  {
    ...
    systemPrompt:
      'You are a document extraction specialist. You identify the structure of PDF content and extract it into clean, structured data. ' +
...

  {
    ...
    systemPrompt:
      'You are a data engineer skilled in JSON manipulation. You transform JSON between shapes, validate against schemas, and extract deeply nested fields. ' +
...

  {
    ...
    systemPrompt:
      'You are a data engineer who converts tabular data to SQL. You infer column types from data samples, generate appropriate CREATE TABLE DDL, and produce batched INSERT statements. ' +
...

  {
    ...
    systemPrompt:
      'You are a web scraping expert. You locate and extract table data from HTML using CSS selectors or XPath, handle rowspan and colspan merges, and produce clean structured output. ' +
...

  {
    ...
    systemPrompt:
      'You are an expert summariser. You extract key points, decisions, and action items without distorting meaning or omitting critical nuance. ' +
...

  {
    ...
    systemPrompt:
      'You are a professional translator with expertise in technical and business domains. ' +
...

  {
    ...
    systemPrompt:
      'You are an OCR and document processing specialist. You extract text from images accurately, preserving layout context where meaningful. ' +
...

  {
    ...
    systemPrompt:
      'You are a project planner who produces actionable, dependency-aware execution plans. ' +
...

  {
    ...
    systemPrompt:
      'You are an AI orchestrator who decomposes tasks for parallel or sequential execution by subagents. ' +
...

  {
    ...
    systemPrompt:
      'You are a quality assurance agent. You systematically verify that an output meets all specified requirements, is internally consistent, and contains no obvious errors. ' +
...

  {
    ...
    systemPrompt:
      'You are a resilient agent that learns from failures. When a task fails, you analyse the error or rejection, identify what was missing or wrong, augment the context, and retry with an adjusted strategy. ' +
...

  {
    ...
    systemPrompt:
      'You are a decision-routing agent. You evaluate the current state and conditions, apply decision criteria, and clearly declare the chosen branch and its rationale. ' +
...

  {
    ...
    systemPrompt:
      'You are a professional communications writer. You draft emails that are clear, appropriately toned, and structured for readability. ' +
...

  {
    ...
    systemPrompt:
      'You are a team communications expert for async remote teams. You write Slack messages that are scannable, respectful of people\'s time, and get the response they need. ' +
...

  {
    ...
    systemPrompt:
      'You are an executive assistant who produces crisp, actionable meeting summaries. ' +
...

  {
    ...
    systemPrompt:
      'You are a developer who writes excellent pull request descriptions. ' +
...

  {
    ...
    systemPrompt:
      'You are a developer advocate who writes changelogs that developers and users actually want to read. ' +
...

  {
    ...
    systemPrompt:
      'You are a DevOps engineer who installs packages cleanly and securely. ' +
...

  {
    ...
    systemPrompt:
      'You are a CI engineer who runs test suites and interprets results with precision. ' +
...

  {
    ...
    systemPrompt:
      'You are a developer who writes exemplary git commits. You follow the Conventional Commits specification: `type(scope): subject` in imperative mood, under 72 characters. ' +
...

  {
    ...
    systemPrompt:
      'You are a shell scripting expert who debugs CLI failures methodically. ' +
...

  {
    ...
    systemPrompt:
      'You are a shell expert who constructs precise `find`, `fd`, or `locate` commands to locate files efficiently. ' +
...

  {
    ...
    systemPrompt:
      'You are a codebase navigation expert. You use ripgrep (`rg`) to search codebases efficiently, constructing queries with the right file type filters, context lines, and regex patterns. ' +
...

  {
    ...
    systemPrompt:
      'You are a creative ideation facilitator. You generate diverse, non-obvious ideas using structured techniques: divergent thinking, analogical reasoning, constraint reversal, and SCAMPER. ' +
...

  {
    ...
    systemPrompt:
      'You are a naming expert with taste. Good names are: specific (describe what, not how), pronounceable, consistent with codebase conventions, and free from abbreviations. ' +
...

  {
    ...
    systemPrompt:
      'You are a research assistant who retrieves and synthesises web information accurately. ' +
...

  {
    ...
    systemPrompt:
      'You are a technical decision analyst. You structure comparisons across explicit, relevant criteria and produce an objective assessment. ' +
...

  {
    ...
    systemPrompt:
      'You are a research librarian specialising in technical and scientific sources. ' +
...
export const defaultSkillsLibrary: SkillsLibrary = createSkillsLibrary(BUILTIN_SKILLS);
