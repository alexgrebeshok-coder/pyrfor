...
export interface SetMyCommandsApi {
  setMyCommands(
    ...
    opts?: { scope?: { type: string }; language_code?: string },
  ): Promise<unknown>;
}

// ─── Canonical command list ───────────────────────────────────────────────────

export const PYRFOR_COMMANDS: BotCommandSpec[] = [
  // default scope only
...
function validateCommand(spec: BotCommandSpec): void {
  if (!spec.command || spec.command.length === 0) {
    throw new Error(`Invalid command: empty string`);
  }
  if (!COMMAND_RE.test(spec.command)) {
    throw new Error(
      `Invalid command "${spec.command}": must be 1-32 chars, lowercase ASCII letters, digits, or underscore`,
    );
  }
  ...
  if (spec.description.length > 256) {
    throw new Error(
      `Invalid description for command "${spec.command}": exceeds 256 chars (${spec.description.length})`,
    );
  }
}
...
type ScopeType = NonNullable<BotCommandSpec['scope']>;

export async function publishBotCommands(
  api: SetMyCommandsApi,
  commands: BotCommandSpec[] = PYRFOR_COMMANDS,
  opts?: { languageCode?: string },
): Promise<{ scopesApplied: number; total: number }> {
  ...
  // Group by scope
  const groups = new Map<ScopeType, Array<{ command: string; description: string }>>();
  for (const spec of commands) {
    const scope: ScopeType = spec.scope ?? 'default';
