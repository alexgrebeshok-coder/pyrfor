 ...
 */
function expandField(token: string, def: FieldDef, fieldIndex: number): Set<number> {
  ...
  if (slashIdx !== -1) {
    ...
    if (!/^\d+$/.test(stepStr)) {
      throw new CronParseError(
        `Invalid step "${stepStr}" in field ${def.name}`, fieldIndex
      );
    }
    step = parseInt(stepStr, 10);
    if (step < 1) {
      throw new CronParseError(
        `Step must be >= 1 in field ${def.name}, got ${step}`, fieldIndex
      );
    }
  } else {
    rangeToken = token;
  ...
  if (rangeToken === '*') {
    ...
    if (parts.length !== 2 || parts[0] === '' || parts[1] === '') {
      throw new CronParseError(
        `Invalid range "${rangeToken}" in field ${def.name}`, fieldIndex
      );
    }
    if (!/^\d+$/.test(parts[0]) || !/^\d+$/.test(parts[1])) {
      throw new CronParseError(
        `Non-numeric range "${rangeToken}" in field ${def.name}`, fieldIndex
      );
    }
    ...
    if (lo > hi) {
      throw new CronParseError(
        `Range start ${lo} > end ${hi} in field ${def.name}`, fieldIndex
      );
    }
  } else {
    // Single value.
    if (!/^\d+$/.test(rangeToken)) {
      throw new CronParseError(
        `Non-numeric value "${rangeToken}" in field ${def.name}`, fieldIndex
      );
    }
...
export function parseCron(expr: string): CronSchedule {
  ...
  const resolved = CRON_PRESETS[trimmed] ?? trimmed;

  const parts = resolved.trim().split(/\s+/);
  if (parts.length !== 5) {
    throw new CronParseError(
      `Expected 5 fields, got ${parts.length}: "${resolved}"`
    );
  }
 ...
 */
export function matches(schedule: CronSchedule, ms: number): boolean {
  const d = new Date(ms);
  ...
  if (!schedule.minutes.has(min))  return false;
  if (!schedule.hours.has(hour))   return false;
...
export function nextRun(
  ...
  throw new CronParseError(
    `No occurrence found within 4 years for schedule "${schedule.raw}"`
  );
}
