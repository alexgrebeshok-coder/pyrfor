...
export interface CallbackRouter {
  register(
    kind: CallbackKind,
    handler: (parsed: ParsedCallback, ctx?: unknown) => Promise<void> | void,
  ): void;
...
function byteLen(s: string): number {
  ...
  for (let i = 0; i < s.length; i++) {
    ...
    else { n += 2; i++; } // surrogate pair → 4 bytes total, counted over two iterations
  }
  return n;
}
...
function truncateBytes(s: string, maxBytes: number): string {
  ...
  for (let i = 0; i < s.length; i++) {
    ...
    else if (c < 0xd800 || c > 0xdfff) w = 3;
    else { w = 2; } // first half of surrogate pair (4 bytes across two chars, skip second below)
...
export function choiceKeyboard(
  ...
  const buttons: InlineButton[] = choices.map((label, idx) => ({
 ...
 */
export function paginatedListKeyboard(
  ...
  const itemRows: InlineButton[][] = slice.map((item) => [
    {
...
export function menuKeyboard(
  items: { label: string; callback: string }[],
  columns = 1,
): InlineKeyboard {
...
type HandlerFn = (parsed: ParsedCallback, ctx?: unknown) => Promise<void> | void;
...
export function createCallbackRouter(): CallbackRouter {
  const handlers = new Map<CallbackKind, HandlerFn>();
  ...
  return {
    ...
    async handle(data, ctx) {
      ...
      try {
        ...
        console.warn(
          `[telegram-keyboards] router handler for "${parsed.kind}" threw:`,
          err,
        );
        return false;
      }
