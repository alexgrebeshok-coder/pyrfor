...
export const WORKER_PROTOCOL_VERSION = 'wp.v2' as const;

export type WorkerProtocolVersion = typeof WORKER_PROTOCOL_VERSION;

export type WorkerFrameType =
  | 'plan_fragment'
  | 'proposed_patch'
  | 'proposed_command'
  | 'request_capability'
...
export type WorkerFrame =
  | PlanFragmentFrame
  | ProposedPatchFrame
  | ProposedCommandFrame
  | RequestCapabilityFrame
  | CheckpointFrame
  | HeartbeatFrame
  | ArtifactReferenceFrame
  | WarningFrame
  | FinalReportFrame
  | FailureReportFrame;
...
export type WorkerFrameValidationResult =
  ...
  | { ok: false; errors: WorkerFrameValidationErrorDetail[] };
...
const WORKER_FRAME_TYPES = new Set<WorkerFrameType>([
  'plan_fragment',
  'proposed_patch',
  'proposed_command',
  'request_capability',
...
const FAILURE_STATUSES = new Set<FailureReportFrame['status']>([
  'failed',
  'policy_blocked',
  'timed_out',
  'cancelled',
...
function isWorkerFrameType(value: unknown): value is WorkerFrameType {
  return typeof value === 'string' && WORKER_FRAME_TYPES.has(value as WorkerFrameType);
}
...
function validateBase(record: Record<string, unknown>): WorkerFrameValidationErrorDetail[] {
...
function validateFrameSpecific(record: Record<string, unknown>): WorkerFrameValidationErrorDetail[] {
  const errors: WorkerFrameValidationErrorDetail[] = [];
  ...
  switch (type) {
    ...
    case 'final_report':
      ...
      break;
    case 'failure_report':
      if (typeof record['status'] !== 'string' || !FAILURE_STATUSES.has(record['status'] as FailureReportFrame['status'])) {
...
function validateNestedString(
  ...
  key: string,
  path: string,
): WorkerFrameValidationErrorDetail[] {
  return typeof record[key] === 'string' && (record[key] as string).length > 0
    ? []
    : [{ path, message: 'must be a non-empty string' }];
}

export function validateWorkerFrame(input: unknown): WorkerFrameValidationResult {
  if (!isRecord(input)) {
  ...
  return { ok: true, frame: input as unknown as WorkerFrame };
}

export function parseWorkerFrame(input: unknown): WorkerFrame {
  const result = validateWorkerFrame(input);
  if (!result.ok) throw new WorkerProtocolValidationError(result.errors);
  return result.frame;
}

export function isWorkerFrame(input: unknown): input is WorkerFrame {
