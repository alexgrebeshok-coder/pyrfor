...
export function buildActorDispatchContextBlock(pack: ContextPack | null | undefined, actorId: string): string | undefined {
  ...
  if (taskSection) lines.push(`task_contract: ${compactJson(taskSection.content, 500)}`);

  const policySection = findSection(pack, 'policy');
  if (policySection) lines.push(`policy: ${compactJson(policySection.content, 500)}`);

  const domainSection = findSection(pack, 'domain_facts');
  if (domainSection) lines.push(`domain_facts: ${compactJson(domainSection.content, 400)}`);
  ...
  if (evidenceItems.length > 0) {
    ...
    lines.push(`run_evidence: ${compactJson(evidenceItems, 1_200)}`);
  }
...
function findSection(pack: ContextPack, id: string): ContextPackSection | undefined {
  return pack.sections.find((section) => section.id === id);
}

function runEvidenceItems(section: ContextPackSection | undefined, actorId: string): unknown[] {
  if (!section || !isRecord(section.content) || !Array.isArray(section.content['items'])) return [];
...
function evidencePriority(item: unknown, actorId: string): number {
  if (!isRecord(item)) return 0;
  let score = 0;
  if (item['actorId'] === actorId) score += 2_000_000_000_000_000;
  if (item['artifactKind'] === 'actor_work_proof') score += 1_000_000_000_000_000;
  if (typeof item['createdAt'] === 'string') score += Date.parse(item['createdAt']) || 0;
  return score;
}
...
function publicEvidenceMetadata(item: unknown): Record<string, unknown> {
...
function copyScalarFields(source: Record<string, unknown>, target: Record<string, unknown>, fields: string[]): void {
...
function safeMetadataString(field: string, value: string): string {
  if (field === 'artifactKind' && isAllowedEvidenceKind(value)) return value;
  if ((field === 'createdAt' || field === 'observedAt') && /^\d{4}-\d{2}-\d{2}T[\d:.]+Z$/.test(value)) return value;
  if ((field.endsWith('Hash') || field === 'sha256' || field === 'headSha' || field === 'commitSha') && /^[a-f0-9]{16,128}$/i.test(value)) return value;
  if (field === 'sourceMode' && ALLOWED_EVIDENCE_SOURCE_MODES.has(value)) return value;
  if ((field === 'status' || field === 'verifierStatus') && ALLOWED_EVIDENCE_STATUSES.has(value)) return value;
  return `[redacted-metadata hash=${createHash('sha256').update(value).digest('hex').slice(0, 16)}]`;
}

function isAllowedEvidenceKind(value: string): boolean {
