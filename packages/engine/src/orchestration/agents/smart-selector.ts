...
export class SmartAgentSelector {
   ...
   */
  async selectAgentAsync(
    task: string,
    options: LlmSelectOptions
  ): Promise<KnownAgentId> {
    const heuristic = this.selectAgent(task) as KnownAgentId;
    if (heuristic !== "main") return heuristic;

    const { router, provider, model, workspaceId, timeoutMs = 4000 } = options;
    const providers = router.getAvailableProviders();
    if (providers.length === 0) return "main";

    const targetProvider = provider && providers.includes(provider) ? provider : providers[0];
    const systemPrompt = `You are a routing classifier. Read the user's task and respond with STRICT JSON: {"agentId":"<id>"}. Valid ids are: ${KNOWN_AGENT_IDS.join(", ")}. Pick the single best id. Do not explain.`;
    ...
    try {
      ...
      if (parsed && (KNOWN_AGENT_IDS as readonly string[]).includes(parsed)) {
        return parsed as KnownAgentId;
      }
      logger.info("smart-selector: LLM returned unknown agent id, using heuristic", {
        raw: raw.slice(0, 200),
      });
      return heuristic;
    } catch (err) {
      logger.warn("smart-selector: LLM fallback failed", {
  ...
  private async withTimeout<T>(promise: Promise<T>, timeoutMs: number): Promise<T> {
    return await Promise.race([
      ...
      new Promise<T>((_, reject) => {
        setTimeout(
          () => reject(new Error(`smart-selector: LLM timed out after ${timeoutMs}ms`)),
          timeoutMs
        );
      }),
    ]);
