...
async fn standalone_engine_is_healthy() -> bool {
        ...
        .build()
    {
        Ok(client) => client,
        Err(e) => {
            eprintln!("[pyrfor-sidecar] Failed to build health-check client: {e}");
            return false;
        }
    };

    match client.get(STANDALONE_ENGINE_HEALTH_URL).send().await {
        Ok(response) if response.status().is_success() => true,
        Ok(response) => {
            eprintln!(
                "[pyrfor-sidecar] Standalone Engine health check returned {}.",
                response.status()
            );
            false
        }
        Err(e) => {
            eprintln!("[pyrfor-sidecar] Standalone Engine health check failed: {e}");
            false
        }
    }
}
...
pub(crate) fn resolve_ollama_binary() -> Option<std::path::PathBuf> {
