...
import { ApiError, getMcpStatus, type McpServerStatus } from '../lib/api';
  ...
  return (
    <div className="mcp-status-panel" data-testid="mcp-servers-panel">
      ...
      <p className="mcp-status-hint">
        Read-only status from the runtime MCP client. Server endpoints and secrets are not exposed here.
        Configure MCP servers in your engine / daemon setup.
      </p>
      {error && (
        ...
        <table className="telemetry-table">
          ...
          <tbody>
            {servers.length === 0 && !error && (
              <tr>
                <td colSpan={3} className="telemetry-panel-muted">No MCP servers registered in this process.</td>
              </tr>
            )}
            {servers.map((s) => (
